...
FASTLINT_FORCE_INSTALL = "FASTLINT_FORCE_INSTALL" in os.environ
IS_WINDOWS = platform.system() == "Windows"
# See ../scripts/build-wheels.sh, which is called from our GHA workflows.
# This script assumes the presence of a fastlint-core binary copied under
# cli/src/fastlint/bin by the caller (the GHA workflow).
...
if WHEEL_CMD in sys.argv:
    ...
    class BdistWheel(bdist_wheel):
        ...
        def get_tag(self):
            ...
            # https://github.com/khulnasoft/fastlint-proprietary/pull/2606/files
            # coupling: fastlint.libsonnet default_python_version
            ...
            # supports glibc and/or libmusl. Fastlint-core is statically compiled,
...
if IS_WINDOWS and not FASTLINT_FORCE_INSTALL:
    raise Exception(
        "Fastlint does not support Windows yet, please try again with WSL "
    )
...
install_requires = [
    ...
    #
    # These specifiers are flexible so fastlint can coexist with other tools.
    # Even though we recommend giving fastlint its own virtualenv
    # (or using the official khulnasoft/fastlint Docker image),
    # many users will first try to install it in their project's virtualenv.
    #
    # Flexibility is achieved by, in order of preference:
    # 1. >=x if you know the earliest version that works with Fastlint
    # 2. >=x,<y if you know the earliest version that works with Fastlint,
    #    and know that a later version breaks Fastlint.
    # 3. ~=x.0 if you don't know the earliest version that works with Fastlint
    #
    ...
    # pyfastlint.propagatedBuildInputs
    "attrs>=21.3",
    "boltons~=21.0",
...
setuptools.setup(
    ...
    url="https://github.com/khulnasoft/fastlint",
    # creates a .exe wrapper on windows
    entry_points={
        "console_scripts": [
            "fastlint = fastlint.console_scripts.entrypoint:main",
            "pyfastlint = fastlint.console_scripts.pyfastlint:main",
        ]
    },
    packages=setuptools.find_packages(where="src"),
    package_dir={"": "src"},
    package_data={"fastlint": [os.path.join("bin", "*")]},
