...
@pytest.mark.kinda_slow
def test_output_highlighting(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _errors = run_fastlint_in_tmp(
        "rules/basic.yaml",
...
@pytest.mark.kinda_slow
def test_output_highlighting__no_color(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _errors = run_fastlint_in_tmp(
        "rules/basic.yaml",
        target_name="basic.py",
        output_format=OutputFormat.TEXT,
...
@pytest.mark.kinda_slow
def test_yaml_capturing(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _errors = run_fastlint_in_tmp(
...
def test_yaml_metavariables(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout, _ = run_fastlint_in_tmp(
        "rules/yaml_key.yaml",
...
@pytest.mark.kinda_slow
def test_quiet_mode_has_empty_stderr(run_fastlint_in_tmp: RunFastlint, snapshot):
    """
...
def test_output_format(run_fastlint_in_tmp: RunFastlint, snapshot, format):
    stdout, _ = run_fastlint_in_tmp(
        "rules/eqeq.yaml",
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_additional_outputs(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout, _ = run_fastlint_in_tmp(
        "rules/eqeq.yaml",
        target_name="basic/stupid.py",
        options=[
...
def test_additional_outputs_with_format_flag(
    run_fastlint_in_tmp: RunFastlint, snapshot, format
):
    stdout, _ = run_fastlint_in_tmp(
        "rules/eqeq.yaml",
        target_name="basic/stupid.py",
...
@pytest.mark.kinda_slow
def test_long_rule_id(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout, _ = run_fastlint_in_tmp(
...
def test_long_rule_id_long_text(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout, _ = run_fastlint_in_tmp(
        "rules/long_rule_id.yaml",
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_omit_inventory(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout, _ = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_omit_experiment(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout, _ = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_junit_xml_output(run_fastlint_in_tmp: RunFastlint, snapshot):
...
@pytest.mark.kinda_slow
def test_json_output_with_dataflow_traces(run_fastlint_in_tmp: RunFastlint, snapshot):
    snapshot.assert_match(
        run_fastlint_in_tmp(
...
#       to debug.
#
# pyfastlint/ofastlint status: ofastlint reports 2 more files that are being
# excluded. They're excluded in both implementations.
@pytest.mark.kinda_slow
@pytest.mark.pysemfail
def test_fastlintignore_ignore_log_report(
    run_fastlint_on_copied_files: RunFastlint, tmp_path, snapshot
):
    shutil.copyfile(
        Path(TARGETS_PATH / "ignores" / ".fastlintignore"), tmp_path / ".fastlintignore"
    )
    ...
    _, stderr = run_fastlint_on_copied_files(
...
# Tolerate a different snapshot with pyfastlint than ofastlint.
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_fastlintignore_ignore_log_report_pyfastlint(
    run_fastlint_on_copied_files: RunFastlint, tmp_path, snapshot
):
    test_fastlintignore_ignore_log_report(
        run_fastlint_on_copied_files, tmp_path, snapshot
    )
...
@pytest.mark.kinda_slow
@pytest.mark.pysemfail
def test_fastlintignore_ignore_log_json_report(
    run_fastlint_on_copied_files: RunFastlint, tmp_path, snapshot
):
    shutil.copyfile(
        Path(TARGETS_PATH / "ignores" / ".fastlintignore"), tmp_path / ".fastlintignore"
    ...
    stdout, _ = run_fastlint_on_copied_files(
        "rules/eqeq-basic.yaml",
        # This set of options is carefully crafted
        # to trigger one entry for most ignore reasons.
        # Note that the print order is non-deterministic,
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_fastlintignore_ignore_log_json_report_pyfastlint(
    run_fastlint_on_copied_files: RunFastlint, tmp_path, snapshot
):
    test_fastlintignore_ignore_log_json_report(
        run_fastlint_on_copied_files, tmp_path, snapshot
    )
...
def test_git_repo_output(
    run_fastlint: RunFastlint, git_repo, tmp_path, monkeypatch, snapshot
):
    """
...
# differs between pyfastlint and ofastlint because it's a location in the rule
# (not in the target), and pyfastlint passes a preprocessed rule file to
# fastlint-core hence the mistmatch.
@pytest.mark.slow
@pytest.mark.osemfail
def test_output_matching_explanations(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout, _ = run_fastlint_in_tmp(
        "rules/eqeq-basic.yaml",
...
@pytest.mark.osemfail
def test_file_count_multifile(run_fastlint_in_tmp: RunFastlint, snapshot, target_dir):
    _, stderr = run_fastlint_in_tmp(
...
@pytest.mark.slow
@pytest.mark.osemfail
def test_output_truncated_messages(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout, _ = run_fastlint_in_tmp(
