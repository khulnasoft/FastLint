...
# NOTE: We need to quarantine at least one test as it is failing with the ofastlint
...
def test_rule_parser__success(run_fastlint_in_tmp: RunFastlint, snapshot, filename):
    run_fastlint_in_tmp(f"rules/syntax/{filename}.yaml")


@pytest.mark.kinda_slow
@pytest.mark.parametrize("filename", syntax_fails)
def test_rule_parser__failure(run_fastlint_in_tmp: RunFastlint, snapshot, filename):
    run_fastlint_in_tmp(f"rules/syntax/{filename}.yaml", assert_exit_code={2, 7, 8})


# TODO: ofastlint does not return exit code 8 yet, since all errors are handled
# by Core_runner, which returns invalid_code (3) or fatal (2), using
# Cli_json_output.exit_code_of_error_type
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_regex_with_bad_language(run_fastlint_in_tmp: RunFastlint, snapshot):
    run_fastlint_in_tmp("rules/syntax/badlanguage.yaml", assert_exit_code=8)


@pytest.mark.kinda_slow
def test_nonexisting_file(run_fastlint_in_tmp: RunFastlint, snapshot):
    run_fastlint_in_tmp("rules/does_not_exist.yaml", assert_exit_code=7)


@pytest.mark.kinda_slow
def test_rule_parser__empty(run_fastlint_in_tmp: RunFastlint, snapshot):
    run_fastlint_in_tmp(f"rules/syntax/empty.yaml", assert_exit_code=7)
...
def test_rule_parser__failure__error_messages(
    run_fastlint_in_tmp: RunFastlint, snapshot, filename
):
...
# https://github.com/khulnasoft/fastlint/issues/1095
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_rule_parser_cli_pattern(run_fastlint_in_tmp: RunFastlint, snapshot):
    # Check json output
    stdout, _ = run_fastlint_in_tmp(
...
# This test should ideally pass with ofastlint, but a key name error is a
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_rule_parser_error_key_name_text(run_fastlint_in_tmp: RunFastlint, snapshot):
    # Check pretty print output
    _, stderr = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_rule_parser_error_metavariable_text(run_fastlint_in_tmp: RunFastlint, snapshot):
    _, stderr = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_rule_parser_error_invalid_key_name_text(run_fastlint_in_tmp: RunFastlint):
    _, stderr = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_rule_parser_fastlint_test_invalid_rule(run_fastlint_in_tmp: RunFastlint):
    _, stderr = run_fastlint_in_tmp(
        f"rules/syntax/invalid-patterns-key.yml",
        subcommand="test",
