...
def run_sentinel_scan(check: bool = True, base_commit: Optional[str] = None):
    ...
    env["FASTLINT_USER_AGENT_APPEND"] = "testing"
...
def assert_err_match(snapshot, output, snapshot_name, replace_base_commit=None):
    if "PYTEST_USE_OFASTLINT" in os.environ:
        # PyFastlint and OFastlint currently display slightly different information
        # messages, causing a mismatch in the stderr output. While we may consider
        # activating this assertion for OFastlint in the future, it will only be
        # feasible once PyFastlint and OFastlint exhibit identical line-by-line
...
def test_no_findings_both(git_tmp_path, snapshot):
    # Test if no findings in head or base fastlint doesnt explode
    foo = git_tmp_path / "foo.py"
    bar = git_tmp_path / "bar.py"
