...
#
# Note that Fastlint should work properly as root, we just don't have tests
...
#
# Pyfastlint fails to ignore the target without read permissions and passes
# it to fastlint-core. This is why --x-ls fails the test. These targets
# are checked again by fastlint-core which then excludes it correctly
@pytest.mark.pysemfail
@pytest.mark.kinda_slow
def test_permissions_ls(run_fastlint_on_copied_files: RunFastlint, snapshot):
    ...
    # files.
    stdout, stderr = run_fastlint_on_copied_files(
...
# End-to-end test. This exercises a regular fastlint scan, in particular
# one using fastlint-core underneath.
#
...
# git doing some filtering of its own. Here, we run fastlint on a non-git
# project and we expect all skipped targets to be reported with ofastlint
# and pyfastlint.
#
def run_test_permissions_scan_full(
    run_fastlint_on_copied_files: RunFastlint, snapshot, verbose: bool
):
    # Giving up on running this as root
    skip_test_if_root()
...
# pyfastlint fails because it doesn't report the skipped folder. This is benign.
#
@pytest.mark.pysemfail
@pytest.mark.kinda_slow
def test_permissions_scan_full_strict(
    run_fastlint_on_copied_files: RunFastlint, snapshot
):
    run_test_permissions_scan_full(run_fastlint_on_copied_files, snapshot, verbose=True)


# Less strict: don't care about whether skipped files or folders are reported.
#
@pytest.mark.kinda_slow
def test_permissions_scan_full_lax(run_fastlint_on_copied_files: RunFastlint, snapshot):
    run_test_permissions_scan_full(run_fastlint_on_copied_files, snapshot, verbose=False)
