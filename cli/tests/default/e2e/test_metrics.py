"""
Tests for fastlint.metrics and associated command-line arguments.
"""
import json
import os
import re
...
- id: python.lang.correctness.useless-eqeq.useless-eqeq
  ...
  metadata:
    ...
    source: https://fastlint.dev/r/python.lang.correctness.useless-eqeq.useless-eqeq
"""
...
def mock_config_request(monkeypatch: MonkeyPatch) -> Iterator[None]:
    monkeypatch.setattr(
        "fastlint.config_resolver.ConfigLoader._download_config_from_url",
...
def test_flags(
    ...
    env = {"FASTLINT_SEND_METRICS": metrics_env} if metrics_env is not None else {}
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_flags_actual_send(run_fastlint_in_tmp: RunFastlint):
    """
...
def test_metrics_payload(tmp_path, snapshot, mocker, monkeypatch, pro_flag):
    ...
    mocker.patch("fastlint.metrics.get_frozen_id", return_value=uuid.UUID("0" * 32))
    mocker.patch("fastlint.metrics.mock_float", return_value=0.0)
    mocker.patch("fastlint.metrics.mock_int", return_value=0)
    ...
    runner = FastlintRunner(
        env={
            ...
            "FASTLINT_INTEGRATION_NAME": "funkyintegration",
        },
