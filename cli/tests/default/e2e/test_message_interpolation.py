...
def test_message_interpolation(run_fastlint_in_tmp: RunFastlint, snapshot, rule, target):
    snapshot.assert_match(
        run_fastlint_in_tmp(rule, target_name=target).stdout, "results.json"
    )


@pytest.mark.slow
def test_no_double_interpolation(run_fastlint_in_tmp: RunFastlint, snapshot):
