...
def test_sarif_output(
    run_fastlint_in_tmp: RunFastlint, snapshot, rule_and_target, dataflow_traces
):
...
@pytest.mark.parametrize(
    "rule_and_target",
    [
        # TODO: ofastlint does not take into account labels
...
# If there are nofastlint comments to ignore findings, SARIF output should
# include them labeled as suppressed.
@pytest.mark.kinda_slow
def test_sarif_output_include_nofastlint(run_fastlint_in_tmp: RunFastlint, snapshot):
    snapshot.assert_match(
        run_fastlint_in_tmp(
            "rules/regex/regex-nofastlint.yaml",
            target_name="basic/regex-nofastlint.txt",
            output_format=OutputFormat.SARIF,
            is_logged_in_weak=True,
        ).stdout,
        "results.sarif",
...
@pytest.mark.kinda_slow
def test_sarif_output_rule_board(run_fastlint_in_tmp: RunFastlint, snapshot):
    snapshot.assert_match(
        run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_sarif_output_with_source(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_sarif_output_with_source_edit(run_fastlint_in_tmp: RunFastlint, snapshot):
    stdout = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_sarif_output_with_nofastlint_and_error(
    run_fastlint_in_tmp: RunFastlint, snapshot
):
    snapshot.assert_match(
        run_fastlint_in_tmp(
            "rules/eqeq.yaml",
            target_name="nofastlint/eqeq-nofastlint.py",
            output_format=OutputFormat.SARIF,
            options=["--error"],
            is_logged_in_weak=True,
        ).stdout,
...
@pytest.mark.kinda_slow
def test_sarif_output_with_autofix(run_fastlint_in_tmp: RunFastlint, snapshot):
    snapshot.assert_match(
        run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_sarif_output_with_dataflow_traces(run_fastlint_in_tmp: RunFastlint, snapshot):
    snapshot.assert_match(
        run_fastlint_in_tmp(
            "rules/taint.yaml",
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_sarif_output_when_errors(run_fastlint_in_tmp: RunFastlint, snapshot):
    snapshot.assert_match(
        run_fastlint_in_tmp(
            "rules/eqeq.yaml",
