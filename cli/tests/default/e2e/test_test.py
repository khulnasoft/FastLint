# Tests for 'fastlint --test' (accessible also now via 'ofastlint test').
# See https://fastlint.dev/docs/writing-rules/testing-rules/ for more info.
#
...
#    the main use case for --test and what we use in fastlint-rules/. However,
#    is a bit harder to test here given how run_fastlint_in_tmp() was
...
#    (maybe we could write this test in Testo instead and for ofastlint-only
...
def test_cli_test_basic(run_fastlint_in_tmp: RunFastlint, snapshot):
...
# a directory (ability heavily used in fastlint-rules/scripts/run-test.sh)
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_cli_test_directory(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _ = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_cli_test_yaml_language(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _ = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
@pytest.mark.osemfail
def test_cli_test_suffixes(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _ = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_cli_test_multiple_annotations(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _ = run_fastlint_in_tmp(
...
# was added in https://github.com/khulnasoft/fastlint/pull/8427 By Austin
@pytest.mark.slow
def test_cli_test_from_entrypoint(snapshot):
    env = {}
    env["PATH"] = os.environ.get("PATH", "")

    cmd = FASTLINT_BASE_SCAN_COMMAND + [
        "--test",
        "--config",
        "rules/basic.yaml",
...
@pytest.mark.kinda_slow
def test_cli_test_ignore_rule_paths(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _ = run_fastlint_in_tmp(
...
@pytest.mark.kinda_slow
def test_cli_todook_filtering(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _ = run_fastlint_in_tmp(
        "rules/basic.yaml",
        options=["--test"],
