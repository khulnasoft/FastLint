...
def test_debug_performance(run_fastlint_in_tmp: RunFastlint):
    """
    Verify that running fastlint with --debug does not result in
    performance slowdown wrt running without --debug
    """
    start_without_debug = time.time()
    run_fastlint_in_tmp("rules/long_message.yaml", target_name="simple.yaml")
    ...
    assert (
        # threshold used to be 0.8 but ofastlint is faster than pyfastlint
        time_without_debug
        > 0.3
    ), (
    ...
    # https://github.com/khulnasoft/fastlint/runs/6468878138?check_suite_focus=true
    # so I've put 0.2
