...
def test_dump_command_for_core():
    fastlint_core_command = subprocess.check_output(
        FASTLINT_BASE_SCAN_COMMAND
        + [
            "--config",
    ...
    result = subprocess.run(fastlint_core_command, shell=True)
...
def test_dump_engine():
    result = subprocess.check_output(
        FASTLINT_BASE_SCAN_COMMAND + ["--dump-engine-path"],
        encoding="utf-8",
    )

    assert re.match(r"/[\w/\_\-\.]+/fastlint-core", result)
