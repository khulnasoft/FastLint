...
def test_publish(tmp_path, mocker):
    ...
    tests_path = Path(TARGETS_PATH / "fastlint-publish" / "valid")
    ...
    assert result.output == "run `fastlint login` before using upload\n"
    ...
    # log back in
    result = runner.invoke(
        cli, subcommand="login", args=[], env={"FASTLINT_APP_TOKEN": "fakeapitoken"}
    )
    ...
    result = runner.invoke(
        ...
        subcommand="publish",
        args=[
            str(Path(TARGETS_PATH / "fastlint-publish" / "invalid").resolve()),
        ],
    )
    assert result.exit_code == 2
    ...
    result = runner.invoke(
        ...
        subcommand="publish",
        args=[
            str(Path(TARGETS_PATH / "fastlint-publish" / "multirule").resolve()),
        ],
    )
    assert result.exit_code == 2
    assert (
    ...
    assert result.exit_code == 2
    assert (
        "Only one public rule can be uploaded at a time: specify a single Fastlint rule"
        in result.output
    )
