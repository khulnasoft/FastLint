...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "rules/dependency_aware/js-pnpm-sca.yaml",
            "dependency_aware/pnpm-v9-no-snapshots-this-is-wrong-btw",
        ),
        (
...
def test_ssc(run_fastlint_on_copied_files: RunFastlint, snapshot, rule, target):
    result = run_fastlint_on_copied_files(rule, target_name=target)
...
def test_ssc__requirements_lockfiles(
    run_fastlint_on_copied_files: RunFastlint, snapshot, rule: str, target: str
):
    """
...
def test_ssc__lockfileless(
    run_fastlint_on_copied_files: RunFastlint, snapshot: Any, rule: str, target: str
):
    """
    Run end-to-end SSC tests with lockfileless resolution enabled.
    """
    result = run_fastlint_on_copied_files(
...
def test_ssc__pypi_package_name_lowercase(
    run_fastlint_on_copied_files: RunFastlint, snapshot, rule, target
):
    """
...
# So we add the keyword arg to run_fastlint and manually do some cd-ing
@pytest.mark.osemfail
def test_no_lockfiles(
    run_fastlint: RunFastlint, monkeypatch: pytest.MonkeyPatch, tmp_path: Path, snapshot
):
