...
def test_basic(run_fastlint_in_tmp: RunFastlint, snapshot, rule, target):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # https://github.com/khulnasoft/fastlint/pull/6900
        # commented for now
        #
        # Let's say you have a pattern `func($X)` to match `func(bar)` where `bar`
        # comes from package `foo`. Fastlint-core returns two matches, in each match
...
@pytest.mark.osemfail
def test_recursive(run_fastlint_in_tmp: RunFastlint, snapshot, rule, target):
    snapshot.assert_match(
        run_fastlint_in_tmp(rule, target_name=target, is_logged_in_weak=True).stdout,
        "results.json",
    )
