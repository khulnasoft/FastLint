...
# The fingerprint should not change across fastlint versions; it must remain
# stable.
...
@pytest.mark.kinda_slow
def test_value(run_fastlint_in_tmp: RunFastlint, snapshot, rule_and_target):
...
# TODO: this currently don't pass with ofastlint, mostly because
# of rule.py formula_string() not being fully ported to ofastlint
@pytest.mark.kinda_slow
@pytest.mark.parametrize(
    "rule_and_target",
...
def test_value_osemfail(run_fastlint_in_tmp: RunFastlint, snapshot, rule_and_target):
    (rule, target) = rule_and_target
    results, _errors = run_fastlint_in_tmp(
        rule,
        target_name=target,
...
@pytest.mark.kinda_slow
def test_duplicate_matches_indexing(run_fastlint_in_tmp: RunFastlint, snapshot):
    results, _errors = run_fastlint_in_tmp(
...
def test_id_change(
    run_fastlint_on_copied_files: RunFastlint, tmp_path, rule, target_name, expect_change
):
    """
    ...
    :param rule: The Fastlint rule that should trigger a finding
    ...
    # target path.
    # Note that b/c we use run_fastlint_on_copied_files, the current working
    # directory for Fastlint is tmp_path; I specify the target as an absolute
    ...
    def run_on_target(subpath):
        ...
        results, _ = run_fastlint_on_copied_files(
            rule,
...
def test_id_change_osemfail(
    ...
    test_id_change(
        run_fastlint_on_copied_files, tmp_path, rule, target_name, expect_change
    )
