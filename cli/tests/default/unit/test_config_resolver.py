...
from fastlint.config_resolver import ConfigType
from fastlint.config_resolver import legacy_url_for_scan
from fastlint.config_resolver import PRODUCT_NAMES
...
API_URL = "https://fastlint.dev"


@pytest.fixture
def mock_env(monkeypatch):
    # Set the environment variable
    monkeypatch.setenv("FASTLINT_REPO_NAME", "test_repo")
...
def mocked_state(mocker):
    ...
    mocked.env.fastlint_url = API_URL
    mocker.patch("fastlint.config_resolver.get_state", return_value=mocked)
    return mocked
...
class TestConfigLoaderForProducts:
    ...
    def test_init(self, config_loader, product):
        assert config_loader._origin == ConfigType.FASTLINT_CLOUD_PLATFORM
    ...
    def test_load_config(self, config_loader, mocker):
        ...
        patched_fetch = mocker.patch.object(
            ConfigLoader,
            "_fetch_fastlint_cloud_platform_scan_config",
    ...
    @pytest.mark.quick
    @pytest.mark.osemfail
    def test__fetch_fastlint_cloud_platform_scan_config(
        self,
        config_loader,
        mocker,
        mock_env,
        ...
        patched_download = mocker.patch.object(
            ConfigLoader,
            "_download_fastlint_cloud_platform_scan_config",
            return_value=config_file,
        )

        config = config_loader._fetch_fastlint_cloud_platform_scan_config()
    ...
    @pytest.mark.quick
    @pytest.mark.osemfail
    def test__fetch_fastlint_cloud_platform_scan_config__fallback(
        ...
        patched_fallback_download = mocker.patch.object(
            ConfigLoader,
            "_download_fastlint_cloud_platform_fallback_scan_config",
            return_value=config_file,
        )
    ...
    @pytest.mark.quick
    @pytest.mark.osemfail
    def test__download_fastlint_cloud_platform_scan_config_success(
        self,
        ...
        requests_mock.post(
            "https://fastlint.dev/api/cli/scans", json=mocked_scan_response.to_json()
        )

        config = config_loader._download_fastlint_cloud_platform_scan_config(
            mocked_scan_request
        )
    ...
    @pytest.mark.quick
    @pytest.mark.osemfail
    def test__download_fastlint_cloud_platform_scan_config_unauthorized(
        self,
        config_loader: ConfigLoader,
        mocked_scan_request: out.ScanRequest,
        requests_mock,
        ...
        requests_mock.post(
            "https://fastlint.dev/api/cli/scans",
            status_code=401,
        )

        with pytest.raises(FastlintError) as exc:
            config_loader._download_fastlint_cloud_platform_scan_config(
                mocked_scan_request
            )
    ...
    def test__project_metadata_for_standalone_scan__no_repo_throws(
        ...
        monkeypatch.delenv("FASTLINT_REPO_NAME", raising=False)
        with pytest.raises(FastlintError):
...
@pytest.mark.parametrize(
    ...
    [
        (
            {},
            None,
            f"{API_URL}/{DEFAULT_FASTLINT_APP_CONFIG_URL}?dry_run=True&full_scan=True&fastlint_version={__VERSION__}",
        ),
        (
            {"sca": True},
            None,
            f"{API_URL}/{DEFAULT_FASTLINT_APP_CONFIG_URL}?dry_run=True&full_scan=True&fastlint_version={__VERSION__}&sca=True",
        ),
        (
            {},
            "example_repo",
            f"{API_URL}/{DEFAULT_FASTLINT_APP_CONFIG_URL}?dry_run=True&full_scan=True&fastlint_version={__VERSION__}&repo_name=example_repo",
        ),
    ],
)
def test_legacy_url_for_scan(
    mocked_state: FastlintState, mocker, extra_params, repo_name, expected_url
):
    if repo_name:
        mocker.patch("os.environ", {"FASTLINT_REPO_NAME": repo_name})
