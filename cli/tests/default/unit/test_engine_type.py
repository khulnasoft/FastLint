...
from fastlint.engine import EngineType as ET
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # fastlint ci with toggle off, full scan
...
def test_decide_engine_type(
    ...
    if is_interfile_flag_on is not None:  # None means we're in `fastlint scan`
        ci_scan_handler = mocker.Mock(spec=ScanHandler)
        ci_scan_handler.deepfastlint = is_interfile_flag_on
    ...
    if is_secrets_scan and engine_flag is ET.OSS:
        pytest.raises(FastlintError, ET.decide_engine_type, *args)
    else:
