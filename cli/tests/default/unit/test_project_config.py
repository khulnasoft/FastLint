...
def test_projectconfig__find_all_config_files_basic(git_tmp_path):
    ...
    make_fastlintconfig_file(git_tmp_path, CONFIG_TAGS)
    config_files = ProjectConfig._find_all_config_files(git_tmp_path, git_tmp_path)
    assert config_files == [git_tmp_path / ".fastlintconfig"]
...
def test_projectconfig__find_all_config_files_monorepo(git_tmp_path):
    ...
    make_fastlintconfig_file(service_1_dir, CONFIG_TAGS_MONOREPO_1)
    # Service 2 config file
    make_fastlintconfig_file(service_2_dir, CONFIG_TAGS_MONOREPO_2)
    ...
    assert git_tmp_path / ".fastlintconfig" in config_files
    # Assert that it only contains config from service1
    assert service_1_dir / ".fastlintconfig" in config_files
    # Assert that it does not contain config from service2
    assert service_2_dir / ".fastlintconfig" not in config_files
...
def test_projectconfig_load_all_basic(git_tmp_path, mocker):
    ...
    mocker.patch("fastlint.git.get_git_root_path", return_value=git_tmp_path)
...
def test_projectconfig_load_from_file_invalid_format(tmp_path):
    tmp_file = tmp_path / ".fastlintconfig"
