...
FAIL_OPEN_URL = "https://fail-open.fastlint.dev/failure"
...
def mock_get_token(mocker):
    mocked = mocker.patch("fastlint.app.auth.get_token", return_value=FAKE_TOKEN)
    yield mocked
...
def mocked_state(mocker, error_handler):
    ...
    mocker.patch("fastlint.state.get_state", return_value=mocked)
    yield mocked
...
def mock_broken_request(requests_mock):
    return requests_mock.get(
        "https://fastlint.dev/api/agent/deployments/current", exc=ConnectionError
    )
...
def fake_command():
    requests.get("https://fastlint.dev/api/agent/deployments/current")
...
def test_send_with_scan_id(
    ...
    error_handler.push_request(
        "get", "https://fastlint.dev/api/agent/deployments/current"
    )
    scan_id = 1234
    ...
    expected_payload = {
        "method": "get",
        "url": "https://fastlint.dev/api/agent/deployments/current",
        "scan_id": 1234,
