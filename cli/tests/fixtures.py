...
# shared across fastlint and fastlint-proprietary, each having their own
# 'conftest.py'.
...
#
if TYPE_CHECKING:
    from tests.conftest import FastlintResult


# The type of run_fastlint functions defined in a project's 'conftest.py'.
# Run 'make typecheck' to run mypy on the project.
#
class RunFastlint(Protocol):
    def __call__(
        self,
        ...
        *,
        ...
        # Context manager to wrap around the fastlint invocation:
        context_manager: Optional[ContextManager] = None,
        is_logged_in_weak: bool = False,
        # The --project-root option is used to prevent the .fastlintignore
        # at the root of the git project to be taken into account when testing,
        # which is a new behavior in ofastlint. It makes fastlint view the
        # project as a non-git project. This option is only used to manage
        # the transition from pyfastlint to ofastlint. Avoid using it if
        # you can.
