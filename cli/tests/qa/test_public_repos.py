...
def assert_sentinel_results(repo_path, sentinel_path, language):
    ...
    # -- Martin
    print(f"fastlint command: {cmd}")

    # nofastlint: python.lang.security.audit.dangerous-subprocess-use.dangerous-subprocess-use
    fastlint_run = subprocess.run(cmd, capture_output=True, encoding="utf-8")
    assert fastlint_run.returncode == 0

    output = json.loads(fastlint_run.stdout)
...
REPO_CACHE = Path(
    os.environ.get("QA_TESTS_CACHE_PATH", appdirs.user_cache_dir("fastlint-qa-tests"))
)
...
#
def test_fastlint_on_repo(monkeypatch, tmp_path, repo):
    ...
    print("--- fastlint error output ---")
    print(res.stderr)
    print("----------------------------")
    print("--- fastlint standard output ---")
    print(res.stdout)
    print("-------------------------------")
