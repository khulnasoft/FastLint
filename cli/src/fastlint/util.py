...
from fastlint.constants import FIXTEST_SUFFIX
from fastlint.constants import YML_SUFFIXES
from fastlint.constants import YML_TEST_SUFFIXES
...
def has_color() -> bool:
    ...
    """
    force_color = is_truthy(os.environ.get("FASTLINT_FORCE_COLOR"))
    # See https://no-color.org/
    no_color = is_truthy(os.environ.get("NO_COLOR")) or is_truthy(
        os.environ.get("FASTLINT_FORCE_NO_COLOR")
    )
...
def with_color(
    ...
    """
    from fastlint.state import get_state  # avoiding circular imports
...
def welcome() -> None:
    """
    Print a welcome message with the Fastlint logo.
    """
...
def sub_check_output(cmd: Sequence[Union[str, Path]], **kwargs: Any) -> Any:
    ...
    # nofastlint: python.lang.security.audit.dangerous-subprocess-use.dangerous-subprocess-use
