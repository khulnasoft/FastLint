...
# Prelude
##############################################################################
# Running a fastlint scan and returning a structured result (RuleMatchMap,
# List[FastlintError], etc.). Most of the CLI arguments processing is done
# in commands/scan.py (but some of the CLI argument sanity checking is
# still done here).
#
# old: this file used to be the entry point of fastlint but became a regular
# file when we introduced fastlint commands (e.g., scan, login). This file
# is now called from commands/scan.py and commands/ci.py instead.
# old: this file used to be called fastlint_main.py
#
import json
import sys
import time
...
import fastlint.error as error
import fastlint.scan_report as scan_report
...
from fastlint.autofix import apply_fixes
...
from fastlint.constants import DEFAULT_DIFF_DEPTH
from fastlint.constants import DEFAULT_TIMEOUT
from fastlint.constants import OutputFormat
from fastlint.constants import TOO_MUCH_DATA
from fastlint.core_runner import CoreRunner
from fastlint.core_runner import Plan
from fastlint.dependency_aware_rule import dependencies_range_match_any
from fastlint.dependency_aware_rule import parse_depends_on_yaml
from fastlint.engine import EngineType
from fastlint.error import InvalidScanningRootError
from fastlint.error import MISSING_CONFIG_EXIT_CODE
from fastlint.error import select_real_errors
from fastlint.error import FastlintError
from fastlint.exclude_rules import filter_exclude_rule
from fastlint.git import BaselineHandler
from fastlint.git import get_project_url
from fastlint.ignores import FileIgnore
from fastlint.ignores import IGNORE_FILE_NAME
from fastlint.ignores import Parser
from fastlint.nofastlint import filter_ignored
from fastlint.output import DEFAULT_SHOWN_SEVERITIES
from fastlint.output import OutputHandler
from fastlint.output import OutputSettings
from fastlint.output_extra import OutputExtra
from fastlint.profile_manager import ProfileManager
from fastlint.resolve_subprojects import resolve_subprojects
from fastlint.rpc_call import dump_rule_partitions
from fastlint.rule import Rule
from fastlint.rule_match import RuleMatches
from fastlint.rule_match import RuleMatchMap
from fastlint.fastlint_interfaces.fastlint_metrics import Any_ as AnySecretsOrigin
from fastlint.fastlint_interfaces.fastlint_metrics import CodeConfig
from fastlint.fastlint_interfaces.fastlint_metrics import SecretsConfig
from fastlint.fastlint_interfaces.fastlint_metrics import SecretsOrigin
from fastlint.fastlint_interfaces.fastlint_metrics import Fastlint as FastlintSecretsOrigin
from fastlint.fastlint_interfaces.fastlint_metrics import SupplyChainConfig
...
from fastlint.fastlint_interfaces.fastlint_output_v1 import Product
from fastlint.fastlint_types import JOIN_MODE
from fastlint.state import get_state
from fastlint.subproject import get_all_source_files
from fastlint.subproject import iter_found_dependencies
from fastlint.subproject import make_dependencies_by_source_path
from fastlint.target_manager import FileTargetingLog
from fastlint.target_manager import SAST_PRODUCT
from fastlint.target_manager import SCA_PRODUCT
...
from fastlint.util import flatten
from fastlint.util import unit_str
...
def get_file_ignore(max_log_list_entries: int) -> FileIgnore:
    ...
    try:
        ...
        workdir = Path.home()
        logger.warn(
            f"Current working directory does not exist! Instead checking {workdir} for .fastlintignore files"
        )

    # Meant to be used only by fastlint-action
    if "FASTLINT_R2C_INTERNAL_EXPLICIT_FASTLINTIGNORE" in environ:
        fastlintignore_path = Path(
            environ["FASTLINT_R2C_INTERNAL_EXPLICIT_FASTLINTIGNORE"]
        ).resolve()
        logger.verbose("Using explicit fastlintignore file from environment variable")
    else:
        fastlintignore_path = Path(workdir / IGNORE_FILE_NAME)
        if not fastlintignore_path.is_file():
            logger.verbose(
            ...
            fastlintignore_path = TEMPLATES_DIR / IGNORE_FILE_NAME
        else:
            logger.verbose("using path ignore rules from user provided .fastlintignore")

    with fastlintignore_path.open() as f:
        file_ignore = FileIgnore.from_unprocessed_patterns(
            base_path=workdir,
            patterns=Parser(file_path=fastlintignore_path, base_path=workdir).parse(f),
...
def file_ignore_to_ignore_profiles(
    ...
    # of .fastlintignore. Don't hardcode this like it is.
    return {
...
def filter_dependency_aware_rules(
    ...
    rules_to_check = [r for r in dependency_aware_rules if r.should_run_on_fastlint_core]
...
# This runs fastlint-core (and also handles SCA and join rules)
@tracing.trace()
def run_rules(
    ...
    strict: bool,
    # TODO: Use an array of fastlint_output_v1.Product instead of booleans flags for secrets, code, and supply chain
    ...
    *,
    ...
    if len(dependency_aware_rules) > 0:
        ...
        # for each subproject, split the errors into fastlint errors and parser errors.
        # output the fastlint errors and store the parser errors for printing in print_scan_status below
        ...
        for subproject in all_subprojects:
            ...
            output_handler.handle_fastlint_errors(
                [
                    error.DependencyResolutionFastlintError(
    ...
    # Dispatching to fastlint-core!
    (
        rule_matches_by_rule,
        fastlint_errors,
        output_extra,
    ) = core_runner.invoke_fastlint_core(
        target_manager,
        rest_of_the_rules,
    ...
    if join_rules:
        import fastlint.join_rule as join_rule
        ...
        for rule in join_rules:
            ...
            output_handler.handle_fastlint_errors(join_rule_errors)

    if len(dependency_aware_rules) > 0:
        from fastlint.dependency_aware_rule import (
            generate_unreachable_sca_findings,
            generate_reachable_sca_findings,
        )

        for rule in dependency_aware_rules:
            if rule.should_run_on_fastlint_core:
                ...
                output_handler.handle_fastlint_errors(dep_rule_errors)
                (
                    dep_rule_matches,
                    dep_rule_errors,
                ) = generate_unreachable_sca_findings(
...
# This is used for testing and comparing with ofastlint.
...
# cli/bin/fastlint -> main.py -> cli.py -> commands/scan.py -> run_scan()
# old: this used to be called fastlint.fastlint_main.main
@tracing.trace()
def run_scan(
    *,
    ...
    respect_fastlintignore: bool = True,
    ...
    logger.debug(f"fastlint version {__VERSION__}")
    ...
    if "FASTLINT_PYTHON_RECURSION_LIMIT_INCREASE" in environ:
        recursion_limit_increase = int(
            environ["FASTLINT_PYTHON_RECURSION_LIMIT_INCREASE"]
        )
    else:
    ...
    if metrics.is_enabled:
        ...
        metrics.add_integration_name(environ.get("FASTLINT_INTEGRATION_NAME"))
        ...
        metrics.add_engine_config(
            ...
            SecretsConfig(
                ...
                else SecretsOrigin(FastlintSecretsOrigin())
            )
    ...
    output_handler.handle_fastlint_errors(config_errors)
    ...
    if not pattern:
        ...
        if len(configs_obj.valid) == 0:
            raise FastlintError(
                """No config given. Run with `--config auto` or see https://fastlint.dev/docs/running-rules/ for instructions on running with a specific config
""",
                code=MISSING_CONFIG_EXIT_CODE,
            )
    ...
    try:
        target_manager = TargetManager(
            ...
            respect_fastlintignore=respect_fastlintignore,
        )
    ...
    fastlint_errors: List[FastlintError] = config_errors + scan_errors
    output_handler.handle_fastlint_errors(fastlint_errors)
        ...
        if not paths_with_matches:
            ...
            try:
                with baseline_handler.baseline_context():
                    ...
                    output_handler.handle_fastlint_errors(baseline_fastlint_errors)
    ...
    if metrics.is_enabled:
        ...
        metrics.add_errors(fastlint_errors)
...
# fastlint)
# old: this used to be called fastlint.fastlint_main.invoke_fastlint()
...
def run_scan_and_return_json(
    ...
    **kwargs: Any,
    ...
    """
    Return Fastlint results of 'config' on 'targets' as a dict|str
