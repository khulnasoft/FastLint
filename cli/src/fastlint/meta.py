...
def get_url_from_sstp_url(sstp_url: Optional[str]) -> Optional[str]:
    ...
    We use repo urls on fastlint-app to link to files, so we need to make sure they are
...
class GitMeta:
    ...
    def repo_name(self) -> str:
        env = get_state().env
        repo_name = os.getenv("FASTLINT_REPO_NAME")
        if repo_name:
            return repo_name
        ...
        if rev_parse.returncode != 0:
            raise Exception(
                "Unable to infer repo_name. Set FASTLINT_REPO_NAME environment variable or run in a valid git project"
            )
    ...
    def repo_display_name(self) -> str:
        # If a subdirectory is passed to fastlint ci, always create a different project
        ...
        return os.getenv("FASTLINT_REPO_DISPLAY_NAME") or display_name

    @property
    def repo_url(self) -> Optional[str]:
        env = get_state().env
        repo_url = os.getenv("FASTLINT_REPO_URL")
        if not repo_url:
            ...
            if git_parse.returncode != 0:
                logger.warn(
                    f"Unable to infer repo_url. Set FASTLINT_REPO_URL environment variable or run in a valid git project with remote origin defined"
                )
    ...
    def commit_sha(self) -> Optional[str]:
        ...
        """
        commit = os.getenv("FASTLINT_COMMIT")
        if commit:
            return commit
    ...
    def ci_job_url(self) -> Optional[str]:
        return os.getenv("FASTLINT_JOB_URL")

    @property
    def pr_id(self) -> Optional[str]:
        return os.getenv("FASTLINT_PR_ID")

    @property
    def pr_title(self) -> Optional[str]:
        return os.getenv("FASTLINT_PR_TITLE")

    @property
    def branch(self) -> Optional[str]:
        branch = os.getenv("FASTLINT_BRANCH")
        if branch:
            return branch
    ...
    def to_project_metadata(self) -> out.ProjectMetadata:
        # Many of these optional fields for the fastlint-app backend depend on git
        ...
        return out.ProjectMetadata(
            # REQUIRED for fastlint-app backend
            repository=self.repo_name,
            repo_display_name=self.repo_display_name,
            # OPTIONAL for fastlint-app backend
...
class GithubMeta(GitMeta):
    ...
    @property
    def repo_name(self) -> str:
        repo_name = os.getenv("FASTLINT_REPO_NAME", os.getenv("GITHUB_REPOSITORY"))
    ...
    def repo_url(self) -> Optional[str]:
        ...
        fastlint_repo_url = os.getenv("FASTLINT_REPO_URL")

        # see NOTE: no-super-first
        if fastlint_repo_url:
            return fastlint_repo_url
        elif self.repo_name:
    ...
    def _find_branchoff_point(self, attempt_count: int = 0) -> str:
        ...
        # workflow needs to explicitly make it available to Fastlint via an
        # environment variable like this:
        #
        # env:
        #   GH_TOKEN: ${{ github.token }}
        #
        # This will allow Fastlint to make this API request even for private
        # repositories.
        if (
            attempt_count == 0
    ...
    @property
    def ci_job_url(self) -> Optional[str]:
        fastlint_job_url = super().ci_job_url
        if fastlint_job_url:
            return fastlint_job_url
    ...
    @property
    def pr_id(self) -> Optional[str]:
        fastlint_pr_id = super().pr_id
        if fastlint_pr_id:
            return fastlint_pr_id
    ...
    @property
    def pr_title(self) -> Optional[str]:
        fastlint_pr_title = super().pr_title
        if fastlint_pr_title:
            return fastlint_pr_title
    ...
    def branch(self) -> Optional[str]:
        ...
        github_ref = os.getenv("FASTLINT_BRANCH", os.getenv("GITHUB_REF"))
        if github_ref:
            return github_ref
        return super().branch
...
class GitlabMeta(GitMeta):
    ...
    @property
    def repo_name(self) -> str:
        project_path = os.getenv("FASTLINT_REPO_NAME", os.getenv("CI_PROJECT_PATH"))
    ...
    @property
    def repo_url(self) -> Optional[str]:
        project_url = os.getenv("FASTLINT_REPO_URL", os.getenv("CI_PROJECT_URL"))
    ...
    @property
    def commit_sha(self) -> Optional[str]:
        commit_sha = os.getenv("FASTLINT_COMMIT", os.getenv("CI_COMMIT_SHA"))
    ...
    @property
    def ci_job_url(self) -> Optional[str]:
        job_url = os.getenv("FASTLINT_JOB_URL", os.getenv("CI_JOB_URL"))
...
class CircleCIMeta(GitMeta):
    ...
    @property
    def ci_job_url(self) -> Optional[str]:
        job_url = os.getenv("FASTLINT_JOB_URL")
        if job_url:
            return job_url
    ...
    @property
    def pr_id(self) -> Optional[str]:
        pr_id = os.getenv("FASTLINT_PR_ID")
        if pr_id:
            return pr_id
...
@dataclass
class FastlintManagedScanMeta(GitMeta):
    """Gather metadata from Fastlint Managed Scanning."""

    environment: str = field(default="fastlint-managed-scan", init=False)

    @property
    def event_name(self) -> str:
        return os.getenv("FASTLINT_MANAGED_SCAN_EVENT_NAME", super().event_name)
...
def generate_meta_from_environment(
    ...
    elif os.getenv("FASTLINT_MANAGED_SCAN") == "true":
        return FastlintManagedScanMeta(baseline_ref)
