"""
This file encapsulates classes necessary in parsing fastlint-core
json output into a typed object.

Not everything is done here though; Some of the parsing
of fastlint-core output is done in core_runner.py (e.g.,
parsing and interpreting the fastlint-core profiling information).

The precise type of the response from fastlint-core is specified in
fastlint_interfaces/fastlint_output_v1.atd
"""
import copy
import dataclasses
...
from fastlint.error import TARGET_PARSE_FAILURE_EXIT_CODE
...
def core_error_to_fastlint_error(err: out.CoreError) -> FastlintCoreError:
    level = err.severity
    ...
    # See https://fastlint.dev/docs/cli-usage/ for meaning of codes
    ...
    return FastlintCoreError(code, level, spans, err)
...
def core_matches_to_rule_matches(
    ...
    # because it is deduplicated in fastlint-core as core_unique_key!
