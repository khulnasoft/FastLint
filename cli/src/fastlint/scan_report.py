...
from fastlint.util import with_feature_status
...
def _print_product_status(sast_enabled: bool = True, sca_enabled: bool = False) -> None:
    ...
    login_command = with_color(Colors.gray, "`fastlint login`")
    ...
    if not is_logged_in:
        message = "\n".join(
            wrap(
                f"💎 Get started with all Fastlint products via {login_command}.",
                width=80,
            )
        ...
        # run then `fastlint ci`. However, there are some additional edge cases to consider
...
def _print_sast_table(
    ...
    # NOTE: ofastlint lacks support for bold table headers
...
def _print_detailed_sca_table(
    ...
    We need to account for several edges cases:
        - `fastlint ci` was invoked but no rules were found (e.g. no lockfile).
        - `fastlint scan` was invoked with the supply-chain flag and no rules found.
        - `fastlint ci` was invoked without the supply-chain flag or feature enabled.
    """
    # 1. Validate that the user is indeed running SCA (and not from fastlint scan).
    is_scan = get_state().is_scan_invocation()
    # 2. Check if the user has metrics enabled.
    metrics = get_state().metrics
    metrics_enabled = metrics.is_enabled
    # If the user has metrics enabled, we can suggest they run `fastlint ci` to get more findings.
    ...
    # Users who have not logged in will not be able to run `fastlint ci`.
    # For users with metrics enabled who are running scan without auth,
    # we should suggest they login and run fastlint ci.
    if is_scan and metrics_enabled:
        login_command = with_color(Colors.gray, "`fastlint login`")
        ci_command = with_color(Colors.gray, "`fastlint ci`")
        if not has_auth:
            message = sep.join(
                wrap(
