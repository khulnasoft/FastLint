"""
Manages system settings for Fastlint

These can change the way Fastlint operates within the environment. They are distinct from config,
which describes which rules Fastlint should run.
...
class Settings:
    ...
    def get_default_path(self) -> Path:
        """
        Uses {$XDG_CONFIG_HOME/fastlint || ~/.fastlint}/settings.yaml unless FASTLINT_SETTINGS_FILE is set
        """
    ...
    # coupling: src/ofastlint/configuring/Fastlint_setting.ml save
    def save(self) -> None:
        try:
