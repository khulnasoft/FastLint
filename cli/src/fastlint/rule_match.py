...
from fastlint.constants import NOSEM_INLINE_COMMENT_RE
from fastlint.constants import RuleScanSource
from fastlint.external.pymmh3 import hash128  # type: ignore[attr-defined]
from fastlint.rule import Rule
from fastlint.fastlint_interfaces.fastlint_output_v1 import Direct
from fastlint.fastlint_interfaces.fastlint_output_v1 import Position
from fastlint.fastlint_interfaces.fastlint_output_v1 import Sha1
...
from fastlint.util import get_lines_from_file
from fastlint.util import get_lines_from_git_blob
...
class RuleMatch:
    ...
    # TODO: reuse fastlint_output_v1.severity instead, or even get rid of
    ...
    # We call rstrip() for consistency with fastlint-core, which ignores whitespace
    ...
    def get_previous_line(self) -> str:
        ...
        This is meant for checking for the presence of a nofastlint comment.
        """
        return (
    ...
    def get_syntactic_context(self) -> str:
        ...
        when `    5 == 5` is updated to `  5 == 5  # nofastlint`,
    ...
    def get_syntactic_id(self) -> str:
        ...
        This value is sent to fastlint.dev and used to track findings across branches
        """
    ...
    def is_validation_state_blocking(self) -> bool:
        ...
        if validation_state_type is out.NoValidator:
            # If there is no validator, we should rely on original dev.fastlint.actions
            return "block" in self.metadata.get("dev.fastlint.actions", ["block"])
        ...
        return (
            self.metadata.get("dev.fastlint.validation_state.actions", {}).get(
                validation_state
            )
            == "block"
        )
    ...
    def is_blocking(self) -> bool:
        ...
        blocking = "block" in self.metadata.get("dev.fastlint.actions", ["block"])
    ...
    def exposure_type(self) -> Optional[str]:
        """
        Mimic the exposure categories on fastlint.dev for supply chain.
    ...
    def to_app_finding_format(
        ...
        # Follow fastlint.dev severity conventions
    ...
    def scan_source(self) -> RuleScanSource:
        src: str = self.metadata.get("fastlint.dev", {}).get("src", "")
        if src == "unchanged":
    ...
    def annotated_rule_name(self) -> str:
        return self.metadata.get("fastlint.dev", {}).get("rule", {}).get("rule_name")
