...
from fastlint.error_location import Position
...
class FastlintError(Exception):
    """
    Parent class of all exceptions we anticipate in Fastlint commands

    All Fastlint Exceptions are caught and their error messages
    ...
    def type_(self) -> out.ErrorType:
        return out.ErrorType(out.FastlintError())
...
def error_type_string(type_: out.ErrorType) -> str:
    ...
    # All the other cases don't have arguments in Fastlint_output_v1.atd
...
def select_real_errors(errors: List[FastlintError]) -> List[FastlintError]:
    return [x for x in errors if is_real_error(x.level)]


@dataclass()
class FastlintCoreError(FastlintError):
    code: int
    level: out.ErrorSeverity
...
class InvalidScanningRootError(FastlintError):
...
class ErrorWithSpan(FastlintError):
    """
    ...
    def __attrs_post_init__(self) -> None:
        if not hasattr(self, "code"):
            raise ValueError("Inheritors of FastlintError must define an exit code")

        if not hasattr(self, "level"):
            raise ValueError("Inheritors of FastlintError must define a level")
    ...
    def format_for_terminal(self) -> str:
        ...
        """
        header = f"{with_color(Colors.red, 'fastlint ' + self.level.to_json())}: {self.short_msg}"
        snippets = []
        for span in self.spans:
            if span.file != "fastlint temp file":
...
# TODO: generalize in a FastlintCliError and just pass the level, code and type_
# and move __str__ special code below in an _error_message like
# for FastlintCoreError
class DependencyResolutionFastlintError(FastlintError):
    """
