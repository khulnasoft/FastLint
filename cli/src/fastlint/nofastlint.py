"""
Handles ignoring Fastlint findings via inline code comments

Currently supports ignoring a finding on a single line by adding a
`# nofastlint:ruleid` comment (or `// nofastlint:ruleid`).

This code currently just consumes the `is_ignored` field, which is set
on each match by the Core Engine.

Coupling: fastlint-action uses a regexp to strip nofastlint comments so as
to normalize the code and not be sensitive to the addition or removal
of nofastlint comments.
See https://github.com/khulnasoft/fastlint-action/blob/develop/src/fastlint_agent/findings.py
...
def filter_ignored(
    ...
    *,
    ...
    :param rule_matches_by_rule: The input findings (typically from a Fastlint call)
