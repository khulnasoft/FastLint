...
class Terminal:
    ...
    def init_for_cli(self) -> None:
        """Call this when fastlint is invoked as a CLI as opposed to a library."""
    ...
    def configure(
        ...
        *,
        ...
        logger = logging.getLogger("fastlint")
        ...
        if force_color or os.environ.get("FASTLINT_FORCE_COLOR") is not None:
            self.force_color_on = True

        if (
            os.environ.get("NO_COLOR") is not None  # https://no-color.org/
            or os.environ.get("FASTLINT_FORCE_NO_COLOR") is not None
        ):
    ...
    def is_color(self) -> bool:
        if self.force_color_on:
            ...
            # while the env var to 'force color' is fastlint-specific
