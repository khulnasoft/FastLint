# This file is DEPRECATED! Please modify instead ofastlint test in
# src/ofastlint/cli_test/Test_subcommand.ml
import collections
import difflib
import functools
import json
...
from fastlint.constants import BREAK_LINE
from fastlint.engine import EngineType
from fastlint.util import final_suffix_matches
from fastlint.util import is_config_fixtest_suffix
from fastlint.util import is_config_suffix
from fastlint.util import is_config_test_suffix
...
# Use ofastlint test --pro if you actually want to process those annotations.
...
def get_expected_and_reported_lines(
    ...
    and the reported lines (which are the lines that the run of fastlint flagged on)
    ...
    for test_file in test_files:
        ...
        for i, line in enumerate(all_lines):
            # +1 because we are 0 based and fastlint output is not, plus skip the comment line
...
def _generate_check_output_line(check_id: str, check_results: Mapping[str, Any]) -> str:
    ...
    if soft_errors:
        ...
        # when running a fastlint scan but fatal in test mode.
...
def generate_test_results(
    *,
    ...
    # COUPLING: the arguments are the same as the second fastlint-core
    # invocation later in this file (except for one)!
    invoke_fastlint_fn = functools.partial(
        invoke_fastlint_multi,
    ...
    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        results = pool.starmap(invoke_fastlint_fn, config_with_tests)
    ...
    # This is the invocation of fastlint for testing autofix.
    # COUPLING: except for autofix, the arguments are the same as above
    invoke_fastlint_with_autofix_fn = functools.partial(
        invoke_fastlint_multi,
        engine_type=engine_type,
        no_git_ignore=no_git_ignore,
        respect_rule_paths=respect_rule_paths,
        ...
        strict=strict,
        optimizations=optimizations,
        # only option that differs from the earlier call to fastlint-core:
        autofix=True,
    )

    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        results = pool.starmap(invoke_fastlint_with_autofix_fn, config_with_tempfiles)
    ...
    if num_tests == 0:
        print(
            "No unit tests found. See https://fastlint.dev/docs/writing-rules/testing-rules"
        )
