...
from fastlint.error import FINDINGS_EXIT_CODE
from fastlint.error import FastlintCoreError
from fastlint.error import FastlintError
from fastlint.formatter.emacs import EmacsFormatter
from fastlint.formatter.gitlab_sast import GitlabSastFormatter
from fastlint.formatter.gitlab_secrets import GitlabSecretsFormatter
from fastlint.formatter.json import JsonFormatter
from fastlint.formatter.junit_xml import JunitXmlFormatter
from fastlint.formatter.sarif import SarifFormatter
from fastlint.formatter.text import TextFormatter
from fastlint.formatter.vim import VimFormatter
...
from fastlint.util import terminal_wrap
...
def _build_time_json(
    ...
    # Can things differ between the targets/rules in pyfastlint and the
    # one actually used in fastlint-core and returned in profile?
...
class OutputHandler:
    ...
    throw an exception.
    If this is normal behavior, the exception _must_ inherit from `FastlintError`.
    ...
    def __init__(
        ...
        self.fastlint_structured_errors: List[FastlintError] = []
        self.error_set: Set[FastlintError] = set()
    ...
    def handle_fastlint_errors(self, errors: Sequence[FastlintError]) -> None:
        timeout_errors = defaultdict(list)
        missing_plugin_errors = []
        for err in errors:
            if (
                isinstance(err, FastlintCoreError)
                and err.is_timeout()
                and err not in self.error_set
            ):
                assert err.core.location
                self.fastlint_structured_errors.append(err)
                ...
                self._handle_fastlint_error(err)

        if timeout_errors and self.settings.has_text_output():
            t_errors = dict(timeout_errors)  # please mypy
            self._handle_fastlint_timeout_errors(t_errors)

        if missing_plugin_errors and self.settings.has_text_output():
            self._handle_fastlint_missing_plugin_errors(missing_plugin_errors)

    def _handle_fastlint_timeout_errors(self, errors: Dict[Path, List[str]]) -> None:
        ...
        for path in errors.keys():
            ...
            if num_errs == self.settings.timeout_threshold:
                error_msg += f"\nFastlint stopped running rules on {path} after {num_errs} timeout error(s). See `--timeout-threshold` for more info."
    ...
    def _handle_fastlint_missing_plugin_errors(self, errors: List[str]) -> None:
        self.has_output = True
        num_errs = len(errors)
        if num_errs >= 1:
    ...
    def _handle_fastlint_error(self, error: FastlintError) -> None:
        """
        Reports generic exceptions that extend FastlintError
        """
        self.has_output = True
        if error not in self.error_set:
            self.fastlint_structured_errors.append(error)
    ...
    def _final_raise(self, ex: Optional[Exception]) -> None:
        if ex is None:
            return
        if isinstance(ex, FastlintError):
            if isinstance(ex.level.value, out.Error_) and not (
                isinstance(ex, FastlintCoreError)
    ...
    # group fastlint core errors by path
    @staticmethod
    def _make_failed_to_analyze(
        fastlint_core_errors: Sequence[FastlintCoreError],
        ...
        return reduce(update_failed_to_analyze, fastlint_core_errors, {})
    ...
    def output(
        ...
        *,
        ...
        if self.final_error:
            ...
            final_error = FastlintError("", code=FINDINGS_EXIT_CODE)
        elif self.fastlint_structured_errors:
            # Assumption: only the fastlint core errors pertain to files; if there are other
            ...
            fastlint_core_errors = [
                err
                for err in self.fastlint_structured_errors
                if isinstance(err, FastlintCoreError)
            ]

            failed_to_analyze_lines_by_path = self._make_failed_to_analyze(
                fastlint_core_errors
            )
            final_error = self.fastlint_structured_errors[-1]
                ...
                if output_destination:
                    ...
                    if output:
                        try:
                            ...
                            # the output string is already pre-formatted by fastlint.console
                            print(output)
        ...
        if self.filtered_rules:
            ...
            if (
                ...
                suggestion_line = "\n(need more rules? `fastlint login` for additional free Fastlint Registry rules)\n"
            stats_line = ""
            if print_summary:
                ...
                if (
                    ...
                    learn_more_line = f"⚡ Supercharge Fastlint OSS when you create a free account at {learn_more_url}."
    ...
    def _post_output(self, output_url: str, output: str) -> None:
        ...
        try:
            ...
            raise FastlintError(f"posting output to {output_url} timed out")
    ...
    def _build_output(
        ...
        cli_paths = out.ScannedAndSkipped(
            # This is incorrect when some rules are skipped by fastlint-core
            # e.g. proprietary rules.
            # TODO: Use what fastlint-core returns for 'scanned' and 'skipped'.
        ...
        if output_format == OutputFormat.TEXT:
            extra["color_output"] = (
                ...
                or os.environ.get("FASTLINT_FORCE_COLOR")
        ...
        # a FastlintError otherwise, but it seems that when fastlint got an error
        ...
        output = formatter.output(  # the rules are used only by the SARIF formatter
            self.rules,
            self.rule_matches,
            self.fastlint_structured_errors,
            cli_output_extra,
            extra,
            self.severities,
            out.FormatContext(
