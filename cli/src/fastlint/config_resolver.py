...
from fastlint.constants import CLI_RULE_ID
from fastlint.constants import Colors
from fastlint.constants import DEFAULT_FASTLINT_APP_CONFIG_URL
from fastlint.constants import ID_KEY
from fastlint.constants import MISSED_KEY
from fastlint.constants import PLEASE_FILE_ISSUE_TEXT
from fastlint.constants import RULES_KEY
from fastlint.error import INVALID_API_KEY_EXIT_CODE
from fastlint.error import InvalidRuleSchemaError
from fastlint.error import FastlintError
from fastlint.error import UNPARSEABLE_YAML_EXIT_CODE
from fastlint.error_location import Span
from fastlint.rule import Rule
from fastlint.rule import rule_without_metadata
from fastlint.rule_lang import EmptySpan
from fastlint.rule_lang import EmptyYamlException
from fastlint.rule_lang import parse_config_preserve_spans
from fastlint.rule_lang import prepend_rule_path
from fastlint.rule_lang import validate_yaml
from fastlint.rule_lang import YamlMap
from fastlint.rule_lang import YamlTree
from fastlint.state import get_state
from fastlint.util import is_config_suffix
from fastlint.util import is_rules
from fastlint.util import is_url
...
CLOUD_PLATFORM_CONFIG_ID = "fastlint-app-rules"
...
class ConfigLoader:
    ...
    def __init__(
        ...
        if config_str == "r2c":
            ...
            self._config_path = f"{state.env.fastlint_url}/{AUTO_CONFIG_LOCATION}"
        else:
    ...
    def load_config(self) -> List[ConfigFile]:
        ...
        elif self._origin == ConfigType.FASTLINT_CLOUD_PLATFORM:
            return [self._fetch_fastlint_cloud_platform_scan_config()]
        else:
            return self._load_config_from_local_path()

    def _nice_fastlint_url(self, url: str) -> str:
        """
        Alters fastlint.dev urls to let user
        click through to the nice display page instead
        of raw YAML.
        Replaces '/c/' in fastlint urls with '/'.
        """
        parsed = urlparse(url)
        if "fastlint.dev" in parsed.netloc and parsed.path.startswith("/c"):
            return url.replace("/c/", "/")
        return url

    def _download_config(self) -> ConfigFile:
        """
        Download a configuration from fastlint.dev
        """
        try:
    ...
    def _load_config_from_local_path(self) -> List[ConfigFile]:
        ...
        if loc.exists():
            if loc.is_file():
                ...
                raise FastlintError(f"config location `{loc}` is not a file or folder!")
        else:
            env = get_state().env
            addendum = ""
            if env.in_docker:
    ...
    def _project_metadata_for_standalone_scan(
        ...
        repo_name = os.environ.get("FASTLINT_REPO_NAME")
        repo_display_name = os.environ.get("FASTLINT_REPO_DISPLAY_NAME", repo_name)

        if repo_name is None:
            if require_repo_name:
                raise FastlintError(
                    f"Need to set env var FASTLINT_REPO_NAME to use `--config {self._config_path}`"
                )
            else:
                repo_name = "unknown"

        return out.ProjectMetadata(
            scan_environment="fastlint-scan",
    ...
    def _fetch_fastlint_cloud_platform_scan_config(self) -> ConfigFile:
        """
        Download a configuration from fastlint.dev using new /api/cli/scans endpoint
        """
        state = get_state()
        ...
        # Require FASTLINT_REPO_NAME env var if SAST or Secrets are requested
        ...
        request = out.ScanRequest(
            scan_metadata=out.ScanMetadata(
                ...
                dry_run=True,  # fastlint scan never submits findings, so always a dry run
            ),
        ...
        try:
            return self._download_fastlint_cloud_platform_scan_config(request)
        except Exception:
            if self._supports_fallback_config:
                try:
                    return self._download_fastlint_cloud_platform_fallback_scan_config()
                except Exception:
                    pass

            raise  # error from first fetch

    def _download_fastlint_cloud_platform_scan_config(
        self, request: out.ScanRequest
    ) -> ConfigFile:
        state = get_state()
        url = f"{state.env.fastlint_url}/api/cli/scans"
        logger.debug("Downloading config from %s", url)
        error = f"Failed to download configuration from {url}"
        try:
            response = state.app_session.post(
                f"{state.env.fastlint_url}/api/cli/scans",
                json=request.to_json(),
            )

            if response.status_code == requests.codes.unauthorized:
                raise FastlintError(
                    "Invalid API Key. Run `fastlint logout` and `fastlint login` again.",
            ...
            try:
                ...
                raise Exception(
                    f"API server at {state.env.fastlint_url} returned this error: {response.text}"
                )
    ...
    def _download_fastlint_cloud_platform_fallback_scan_config(self) -> ConfigFile:
        """
        This function decides what fallback url to call if the fastlint cloud platform
...
) -> Tuple[Dict[str, YamlTree], List[FastlintError]]:
    """
        ...
        try:
            if not config_id:  # registry rules don't have config ids
                ...
                config_id = (
                    ...
                    and (
                        remote_rule_netloc.endswith(".fastlint.dev")
                        or remote_rule_netloc == "fastlint.dev"
                    )
...
class Config:
    ...
    ) -> Tuple["Config", List[FastlintError]]:
    ...
    def from_config_list(
        ...
        for i, config in enumerate(configs):
            try:
                ...
                # https://github.com/khulnasoft/fastlint/issues/1912
    ...
    def get_rules(self, no_rewrite_rule_ids: bool) -> List[Rule]:
        ...
        # behavior as ofastlint i.e. the first occurrence of each rule
    ...
    def _validate(
    ...
    ) -> Tuple[Mapping[str, Sequence[Rule]], List[FastlintError], int]:
        """
        ...
        for config_id, config_yaml_tree in config_dict.items():
            ...
            if not isinstance(config, YamlMap):
                errors.append(FastlintError(f"{config_id} was not a mapping"))
                continue
            ...
            for rule_dict in rules.value:
                try:
                    ...
                    if (
                        ...
                        # code.
                        rule.metadata.get("fastlint.dev", {}).get("rule", {})[
                            "origin"
                        ] = "local"
...
def manual_config(
    ...
    This is used when someone calls `fastlint scan -e print -l py`
    """
    ...
    error_span = Span.from_string(
        f"Fastlint bug generating manual config {PLEASE_FILE_ISSUE_TEXT}", filename=None
    )
    rules_key = {
        ID_KEY: CLI_RULE_ID,
...
def parse_config_string(
    ...
    try:
        fd, rules_tmp_path = mkstemp(suffix=".rules", prefix="fastlint-", text=True)
...
def registry_id_to_url(registry_id: str) -> str:
    """
    Convert from registry_id to fastlint.dev url
    """
    env = get_state().env
    return f"{env.fastlint_url}/{registry_id}"
...
def url_for_policy() -> str:
    ...
    if repo_name is None:
        raise FastlintError(
            "Need to set env var FASTLINT_REPO_NAME to use `--config policy`"
        )
    ...
    params = {
        ...
        "full_scan": True,
        "repo_name": repo_name,
        "fastlint_version": __VERSION__,
    }
    params_str = urlencode(params)
    return f"{env.fastlint_url}/{DEFAULT_FASTLINT_APP_CONFIG_URL}?{params_str}"
...
def legacy_url_for_scan(extra_params: Optional[dict] = None) -> str:
    ...
    if "FASTLINT_REPO_NAME" in os.environ:
        params["repo_name"] = os.environ.get("FASTLINT_REPO_NAME")
...
def get_config(
    ...
    *,
    ...
    if pattern:
        if not lang:
            raise FastlintError("language must be specified when a pattern is passed")
