...
from fastlint.constants import DEFAULT_MAX_MEMORY_PRO_CI
from fastlint.constants import DEFAULT_PRO_TIMEOUT_CI
from fastlint.error import FastlintError
from fastlint.meta import GitMeta
from fastlint.fastlint_core import FastlintCore
from fastlint.fastlint_interfaces import fastlint_output_v1 as out
...
class EngineType(Enum):
    ...
    def decide_engine_type(
        ...
        """Determine which Fastlint engine type to run with.

        Takes into account the following rules:
        - The CLI flag > Fastlint Cloud Platform settings > defaults
        - Requesting the secrets engine implies requesting PRO_INTRAFILE
        - By default, logged in `fastlint ci` scans use PRO_INTRAFILE
        ...
        """
        interfile_is_requested_via_app = ci_scan_handler and ci_scan_handler.deepfastlint
        ...
        if engine_flag is not None:
            ...
            # - ci_scan_handler indicates that `fastlint ci` was the entrypoint
    ...
    def validate_requested_engine(
        ...
        if run_secrets and requested_engine is EngineType.OSS:
            ...
            raise FastlintError("Fastlint Secrets is not part of the open source engine")
    ...
    def get_binary_path(self) -> Optional[Path]:
        if self.is_pro:
            if self.check_is_correct_pro_version():
                return FastlintCore.pro_path()
            else:
                return None
        else:
            return FastlintCore.path()

    # Checks the version stamp that is installed alongside the
    # fastlint-core-proprietary binary to ensure that fastlint-core-proprietary
    # was installed by the current version of Fastlint.
    #
    # See also commands/install.py add_fastlint_pro_version_stamp.
    def check_is_correct_pro_version(self) -> bool:
        version_stamp_path = FastlintCore.pro_version_stamp_path()
