...
from fastlint.error import OK_EXIT_CODE
from fastlint.error import FastlintCoreError
from fastlint.error import FastlintError
from fastlint.error_location import SourceTracker
from fastlint.error_location import Span
from fastlint.rpc_call import validate as rpc_validate
...
class RuleSchema:
    ...
    def get(cls) -> Dict[str, Any]:
        ...
        if not cls._schema:
            yaml = YAML()
            schema_path = (
                Path(__file__).parent / "fastlint_interfaces" / "rule_schema_v1.yaml"
            )
    ...
    class SpanPreservingRuamelConstructor(RoundTripConstructor):
        def construct_object(self, node: Node, deep: bool = False) -> YamlTree:
            ...
            if r is None and not allow_null:
                # This was originally intended only for parsing fastlint rules
                ...
                raise InvalidRuleSchemaError(
                    ...
                    long_msg="In fastlint YAML configuration, null values are prohibited",
                    spans=[
                        Span.from_node(
                            node, source_hash=source_hash, filename=filename
                        ).with_context(before=1, after=1)
...
) -> Tuple[YamlTree, List[FastlintError]]:
...
def sanitize_rule_id_fragment(s: str) -> str:
    ...
    This removes characters that aren't allowed in Fastlint rule IDs.
        ...
        for rule in rules_value:
            ...
            if "min-version" in rule_value:
                ...
                if Version(__VERSION__) < Version(min_version_value):
                    ...
                    # fastlint-core/ofastlint.
                    msg = (
                        f"This rule requires upgrading Fastlint from version "
            ...
            if "max-version" in rule_value:
                ...
                if Version(__VERSION__) > Version(max_version_value):
                    msg = (
                        f"This rule is no longer supported by Fastlint. "
                        f"The last compatible version was {max_version_value}. "
                        f"This version of Fastlint is {__VERSION__}"
                    )
...
def run_rpc_validate(rules_tmp_path: str) -> Literal[True]:
    try:
        ...
        logger.debug(f"fastlint-core validation response: {valid=}")
        if valid:
            logger.debug("fastlint-core validation succeeded")
            return True
        raise RpcValidationError("fastlint-core validation failed")
...
def validate_yaml(
    ...
    try:
        if skip_rpc_validation:
            logger.debug(
                "Skipping fastlint-core validation to proceed directly to jsonschema validation"
            )
        else:
