...
import fastlint.rpc_call as rpc_call
...
from fastlint.fastlint_interfaces.fastlint_output_v1 import DependencyMatch
...
from fastlint.fastlint_interfaces.fastlint_output_v1 import Pypi
...
from fastlint.subproject import find_closest_resolved_subproject
from fastlint.subproject import iter_dependencies
...
def parse_depends_on_yaml(entries: List[Dict[str, str]]) -> Iterator[out.ScaPattern]:
    ...
    for entry in entries:
        ...
        if namespace is None:
            raise FastlintError(f"project-depends-on is missing `namespace`")
        try:
            ecosystem = Ecosystem.from_json(namespace.lower())
        except ValueError:
            raise FastlintError(f"unknown package ecosystem: {namespace}")
        package = entry.get("package")
        if package is None:
            raise FastlintError(f"project-depends-on is missing `package`")
        semver_range = entry.get("version")
        if semver_range is None:
            raise FastlintError(f"project-depends-on is missing `version`")
        try:
            SpecifierSet(semver_range)
        except InvalidSpecifier:
            raise FastlintError(f"invalid semver range {semver_range}")
...
def generate_unreachable_sca_findings(
    ...
    dep_rule_errors: List[FastlintError] = []
        ...
        for subproject in resolved_deps.get(ecosystem, []):
            ...
            for dep_pat, found_dep in dependency_matches:
                ...
                sca_match = out.ScaMatch(
                    ...
                    reachable=False,
                    reachability_rule=rule.should_run_on_fastlint_core,
            ...
            if x_tr:
                ...
                fd, rules_tmp_path = mkstemp(
                    suffix=".rules", prefix="fastlint-", text=True
                )
...
def generate_reachable_sca_findings(
    ...
    List[RuleMatch], List[FastlintError], Callable[[Path, FoundDependency], bool]
]:
    depends_on_keys = rule.project_depends_on
    dep_rule_errors: List[FastlintError] = []
