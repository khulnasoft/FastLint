...
from fastlint.util import format_bytes
from fastlint.util import get_lines_from_file
from fastlint.util import MASK_CHAR
from fastlint.util import MASK_SHOW_PCT
from fastlint.util import truncate
...
def match_to_lines(
    ...
    try:
        ...
        # file used for web framework analysis. The issue is specific to pyfastlint, as
        # for ofastlint, the temporary file isn’t deleted until ofastlint finishes.
        lines = [content]
    yield from format_lines(
        path,
        location.start.line,
...
def print_time_summary(
    time_data: out.Profile, error_output: Sequence[FastlintError]
) -> None:
    items_to_show = 5
    col_lim = 50
    ...
    fastlint_core_errors = [
        err for err in error_output if isinstance(err, FastlintCoreError)
    ]
    errors = {
        (
        ...
        for err in fastlint_core_errors
    }
    ...
    # Consider: get a report from fastlint-core on what language each
    #           file was analyzed as
    # However, this might make it harder for users to confirm
    # fastlint counts against their expected file counts
    ...
    # Output fastlint-core information
    console.print("\nFastlint-core time:")
    console.print(
        ...
        with force_quiet_off(console), console.capture() as captured_output:
            ...
            for group, matches in grouped_matches.items():
                ...
                if group[1] == "generic-secrets" and ctx.is_ci_invocation:
                    url = get_state().env.fastlint_url
                    console.print(
                        textwrap.dedent(
                            f"""
                        Your deployment has generic secrets enabled. {len(matches)} potential line locations
                        will be uploaded to the Fastlint platform and then analyzed by Fastlint Assistant.
                        Any findings that appear actionable will be available in the Fastlint Platform.
            ...
            if cli_output_extra.time:
                print_time_summary(cli_output_extra.time, fastlint_structured_errors)
