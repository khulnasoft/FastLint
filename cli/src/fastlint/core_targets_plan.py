...
# Prelude
##############################################################################
# Generate what will be passed to fastlint-core via --targets
# and specified now in fastlint_output_v1.atd
import collections
...
from fastlint.subproject import get_display_paths
...
class Task:
    ...
    # fastlint-core no longer uses the rule_nums field.
    ...
    def to_target(self) -> out.Target:
        # Once we start sending supply chain rules to fastlint-core,
...
class Plan:
    ...
    to_json: creates the json passed to fastlint_core -
    ...
    def to_targets(self) -> out.Targets:
        """Produce the input to fastlint-core in the form of a list of target files"""
        return out.Targets(
    ...
    def table_by_origin(
        ...
        origin_counts = collections.Counter(
            get_path(rule.metadata, ("fastlint.dev", "rule", "origin"), default="custom")
            for rule in self.rules
