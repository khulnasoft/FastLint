...
def make_login_url() -> Tuple[uuid.UUID, str]:
    ...
    return (
        session_id,
        f"{env.fastlint_url}/login?cli-token={session_id}&docker={env.in_docker}&gha={env.in_gh_action}",
    )
...
def login() -> NoReturn:
    """
    Obtain and save credentials for fastlint.dev

    Looks for an fastlint.dev API token in the environment variable FASTLINT_APP_TOKEN.
    ...
    if saved_login_token and saved_login_token != state.env.app_token:
        click.echo(
            f"API token already exists in {state.settings.path}. To login with a different token logout use `fastlint logout`"
        )
    ...
    if not auth.is_a_tty():
        click.echo(
            f"Error: fastlint login is an interactive command: run in an interactive terminal (or define FASTLINT_APP_TOKEN)",
            err=True,
        )
        sys.exit(FATAL_EXIT_CODE)
    ...
    click.echo(
        "Login enables additional proprietary Fastlint Registry rules and running custom policies from Fastlint Cloud Platform."
    )
    click.echo(f"Opening login at: {url}")
    click.launch(url)
    click.echo(
        "\nOnce you've logged in, return here and you'll be ready to start using new Fastlint rules."
    )
    ...
    for _ in range(MAX_RETRIES):
        r = state.app_session.post(
            f"{state.env.fastlint_url}/api/agent/tokens/requests",
            headers={
                "User-Agent": str(state.app_session.user_agent),
                "X-Fastlint-Client-Id": str(
        ...
        if r.status_code == 200:
            ...
            click.echo(
                f"Unexpected failure from {state.env.fastlint_url}: status code {r.status_code}; please contact support@khulnasoft.com if this persists",
                err=True,
            )
    ...
    click.echo(
        f"Failed to login: please check your internet connection or contact support@khulnasoft.com",
        err=True,
    )
    sys.exit(FATAL_EXIT_CODE)
...
def save_token(login_token: Optional[str], echo_token: bool) -> bool:
    ...
    if login_token is not None and auth.get_deployment_from_token(login_token):
        ...
        click.echo(
            f"Note: You can always generate more tokens at {state.env.fastlint_url}/orgs/-/settings/tokens"
        )
        return True
    else:
