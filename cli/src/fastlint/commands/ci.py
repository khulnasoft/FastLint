...
import fastlint.app.auth as auth
import fastlint.rpc_call
import fastlint.run_scan
import fastlint.fastlint_interfaces.fastlint_output_v1 as out
from fastlint import tracing
from fastlint.app.project_config import ProjectConfig
from fastlint.app.scans import ScanHandler
from khulnasoft.commands.install import run_install_fastlint_pro
from khulnasoft.commands.scan import collect_additional_outputs
from khulnasoft.commands.scan import scan_options
from khulnasoft.commands.wrapper import handle_command_errors
from fastlint.console import console
from fastlint.console import Title
...
from fastlint.git import is_git_repo_empty
from fastlint.git import is_git_repo_root_approx
from fastlint.ignores import IGNORE_FILE_NAME
from fastlint.meta import generate_meta_from_environment
from fastlint.meta import GithubMeta
from fastlint.meta import GitMeta
from fastlint.metrics import MetricsState
...
from fastlint.target_manager import ALL_PRODUCTS
...
ALWAYS_EXCLUDE_PATTERNS = [".fastlint/", ".fastlint_logs/"]

# These patterns are excluded via --exclude unless the user provides their own .fastlintignore
...
def is_valid_pattern(pattern: str) -> bool:
    """
    Parses patterns from fastlint.dev and returns the lines that
...
def get_exclude_paths(
    ...
    for product in ALL_PRODUCTS:
        ...
        # .fastlintignore. But, we've had this for a while, so leaving it not to
...
@click.option(
    ...
    "suppress_errors",
    default=True,
    envvar="FASTLINT_SUPPRESS_ERRORS",
)
@click.option(
    "--subdir",
...
def ci(
    ...
    *,
    ...
    with tracing.TRACER.start_as_current_span("khulnasoft.commands.ci"):
        ...
        if subdir:
            ...
            try:
                ...
                logger.info(
                    "`fastlint ci --subdir` must be given a directory that is actually a subdirectory of the current directory"
                )
                sys.exit(FATAL_EXIT_CODE)

        if not is_git_repo_root_approx():
            logger.info(
                "WARNING: `fastlint ci` is meant to be run from the root of a git repo.\nWhen `fastlint ci` is not run from a git repo, it will not be able to perform all operations.\nWhen `fastlint ci` is run from a git repo, but not the root, links in the uploaded findings may be broken.\n\nTo run `fastlint ci` on only a subdirectory of a git repo, see `--subdir`."
            )
        ...
        if not token and not config:
            ...
            logger.info(
                "run `fastlint login` before using `fastlint ci` or use `fastlint scan` and set `--config`"
            )
            ...
            logger.info(
                "Cannot run `fastlint ci` with --config while logged in. The `fastlint ci` command will upload findings to fastlint-app and those findings must come from rules configured there. Drop the `--config` to use rules configured on fastlint.dev or log out."
            )
            sys.exit(FATAL_EXIT_CODE)
        elif token:
        ...
        # Empty repo case:
        # `fastlint ci` should report success if we are scanning an empty repo;
        ...
        if is_git_repo_empty():
            ...
            contributions = fastlint.rpc_call.contributions()
            if scan_handler:
                with Progress(
        ...
        debugging_table.add_row(
            "versions",
            "-",
            f"fastlint [bold]{fastlint.__VERSION__}[/bold] on python [bold]{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}[/bold]",
        )
        debugging_table.add_row(
            "environment",
            "-",
        ...
        try:
            ...
            if scan_handler:
                ...
                if scan_handler.rules == '{"rules":[]}' and set(
                    ...
                    console.print(
                        f"No rules configured. Visit {state.env.fastlint_url}/orgs/-/policies to configure rules to scan your code.\n"
                    )
        ...
        if run_secrets_flag and requested_engine is EngineType.OSS:
            logger.info(
                "The --secrets and --oss-only flags are incompatible. Fastlint Secrets is a proprietary extension of Open Source Fastlint."
            )
            sys.exit(FATAL_EXIT_CODE)
        ...
        if engine_type.is_pro:
            ...
            if run_secrets:
                console.print("Fastlint Secrets requires Fastlint Pro Engine")
            if engine_type.check_if_installed():
                console.print(
                    f"Using Fastlint Pro Version: [bold]{engine_type.get_pro_version()}[/bold]",
                    markup=True,
                )
                console.print(
        ...
        # performed by the python. Once ofastlint is complete we need only combine
        ...
        try:
            ...
            if scan_handler:
                if not scan_handler.enabled_products:
                    raise FastlintError(
                        "No products are enabled for this organization. Please enable a product in the Settings > Deployment tab of Fastlint Cloud Platform or reach out to support@khulnasoft.com for assistance."
                    )
            ...
            (
                ...
                _missed_rule_count,
                all_subprojects,
            ) = fastlint.run_scan.run_scan(**run_scan_args)
            ...
            output_handler.handle_fastlint_errors([e])
        ...
        if run_historical_secrets_scan and metadata.merge_base_ref:
            ...
            try:
                ...
                fastlint_errors.extend(historical_fastlint_errors)
        ...
        # Since we keep nofastlint disabled for the actual scan, we have to
        ...
        if scan_handler is None:
            # This is a slightly weird case, where we're running `fastlint ci` while logged out with an explicitly provided config
        ...
        if scan_handler:
            ...
            # We upload it separately, and outsource to `ofastlint` so we don't duplicate
            # the implementation.
            if (
                ...
                fastlint.rpc_call.upload_symbol_analysis(
            ...
            if not dry_run:
                if complete_result.success:
                    logger.info("  View results in Fastlint Cloud Platform:")
                else:
                    logger.info(
                        "  Fastlint Cloud Platform is still processing the results of the scan, they will be available soon:"
                    )

                ref_if_available = f"&ref={metadata.branch}" if metadata.branch else ""

                logger.info(
                    f"    {state.env.fastlint_url}/orgs/{scan_handler.deployment_name}/findings?repo={metadata.repo_display_name}{ref_if_available}"
                )
                if "r2c-internal-project-depends-on" in scan_handler.rules:
                    logger.info(
                        f"    {state.env.fastlint_url}/orgs/{scan_handler.deployment_name}/supply-chain/vulnerabilities?repo={metadata.repo_display_name}{ref_if_available}"
                    )
        ...
        if cli_suggested_exit_code == 1 or app_policy_block:
            ...
            if app_other_block and not audit_mode:
                ...
                logger.info(
                    f"  fastlint.dev is suggesting a non-zero exit code ({complete_result.app_block_reason})"
                )
                exit_code = 1
            else:
        ...
        if enable_version_check:
            from fastlint.app.version import version_check
