...
from fastlint.test import get_config_filenames
from fastlint.test import get_config_test_filenames
...
@click.option(
    ...
    " If 'public', rules will be published to the Fastlint Registry (requires --registry-id)",
)
@click.option(
    "--registry-id",
    "registry_id",
...
def publish(
    ...
    """
    Upload rule to fastlint.dev

    Must be logged in to use; see `fastlint login`
    """
    app_session = get_state().app_session

    if not app_session.token:
        click.echo("run `fastlint login` before using upload", err=True)
        sys.exit(FATAL_EXIT_CODE)

    fail_count = 0
    config_filenames, config_test_filenames = _get_test_code_for_config(Path(target))
    if len(config_filenames) == 0:
        click.echo(f"No valid Fastlint rules found in {target}", err=True)
        sys.exit(FATAL_EXIT_CODE)
    if len(config_filenames) != 1 and visibility == VisibilityState.PUBLIC:
        click.echo(
            f"Only one public rule can be uploaded at a time: specify a single Fastlint rule",
            err=True,
        )
        sys.exit(FATAL_EXIT_CODE)
...
def _upload_rule(
    ...
    response = state.app_session.post(
        f"{state.env.fastlint_url}/api/registry/rules", json=request_json
    )
    ...
    if not response.ok:
        ...
        if visibility == VisibilityState.PUBLIC:
            ...
            click.echo(
                f"    Published {visibility} rule at {state.env.fastlint_url}/s/{created_rule['id']}"
            )
        else:
            click.echo(
                f"    Published {visibility} rule at {state.env.fastlint_url}/r/{created_rule['path']}"
            )
