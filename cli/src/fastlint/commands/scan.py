...
import fastlint.config_resolver
import fastlint.run_scan
import fastlint.test
from fastlint import __VERSION__
from fastlint import bytesize
from fastlint import tracing
from fastlint.app.version import get_no_findings_msg
from fastlint.app.version import get_too_many_findings_msg
from fastlint.app.version import TOO_MANY_FINDINGS_THRESHOLD
from khulnasoft.commands.install import determine_fastlint_pro_path
...
from fastlint.constants import DEFAULT_MAX_CHARS_PER_LINE
from fastlint.constants import DEFAULT_MAX_LINES_PER_FINDING
from fastlint.constants import DEFAULT_MAX_LOG_LIST_ENTRIES
from fastlint.constants import DEFAULT_MAX_TARGET_SIZE
...
from fastlint.notifications import possibly_notify_user
...
from fastlint.target_manager import write_pipes_to_disk
from fastlint.util import abort
from fastlint.util import is_truthy
...
# fastlint.run_scan.run_scan() in this file.
_scan_options: List[Callable] = [
    ...
    click.option(
        "--baseline-commit",
        envvar=["FASTLINT_BASELINE_COMMIT", "FASTLINT_BASELINE_REF"],
    ),
    click.option(
        "--metrics",
        "metrics",
        type=METRICS_STATE_TYPE,
        envvar="FASTLINT_SEND_METRICS",
    ),
    optgroup.group(
        "Path options",
    ),
    # temporary option, specific to pyfastlint. Will be removed
    # once everyone is happy with Fastlintignore v2 (OCaml file targeting)
    optgroup.option(
        "--fastlintignore-v2/--no-fastlintignore-v2",
        "use_fastlintignore_v2",
        is_flag=True,
        default=False,
    ),
    optgroup.option(
    ...
    optgroup.option(
        ...
        is_flag=True,
        default=True,
        envvar="FASTLINT_ENABLE_VERSION_CHECK",
    ),
    optgroup.option(
        "-j",
        "--jobs",
    ...
    # TODO: Move to Fastlint Pro Engine group ?
    optgroup.option(
        "--interfile-timeout",
        type=int,
    ),
    ...
    optgroup.group("Fastlint Pro Engine options"),
    optgroup.option(
        "--pro",
        "requested_engine",
        type=EngineType,
...
def scan(
    *,
    ...
    verbose: bool,
    version: bool,
    x_ignore_fastlintignore_files: bool,
    x_ls: bool,
    x_ls_long: bool,
    x_tr: bool,
    path_sensitive: bool,
    allow_local_builds: bool,
) -> Optional[Tuple[RuleMatchMap, List[FastlintError], List[Rule], Set[Path]]]:
    if version:
        print(__VERSION__)
    ...
    if run_secrets_flag and requested_engine is EngineType.OSS:
        abort(
            "Cannot run secrets scan with OSS engine (--oss specified). Fastlint Secrets is a proprietary extension."
        )
    ...
    if trace_endpoint and not trace:
        logger.warning(
            with_color(
                Colors.yellow,
                "The --trace-endpoint flag or FASTLINT_OTEL_ENDPOINT environment variable is specified without --trace.\n"
                "If you intend to enable tracing, please also add the --trace flag.",
            )
        )
    state.traces.configure(trace, trace_endpoint)
    with tracing.TRACER.start_as_current_span("khulnasoft.commands.scan"):
        ...
        # this is useful for our CI job to find where fastlint-core (or fastlint-core-proprietary)
        # is installed and check if the binary is statically linked.
        if dump_engine_path:
            if engine_type == EngineType.OSS:
                print(FastlintCore.path())
            else:
                print(determine_fastlint_pro_path())
            return None
        ...
        # to capture the stderr of fastlint-core or to let fastlint-core reuse
        # the stderr of pyfastlint to display logs as soon as they are produced
        # pyfastlint-only: not needed for ofastlint obviously
        ...
        if include and exclude:
            logger.warning(
                with_color(
                    Colors.yellow,
                    "Paths that match both --include and --exclude will be skipped by Fastlint.",
                )
            )
        ...
        if not scanning_roots:
            fastlint.config_resolver.adjust_for_docker()
        ...
        if test:
            ...
            # machinery to evaluate fastlint performance) uses
            # managed_output internally
            fastlint.test.test_main(
        ...
        with tempfile.TemporaryDirectory() as pipes_dir:
            ...
            return_data: Optional[
                Tuple[RuleMatchMap, List[FastlintError], List[Rule], Set[Path]]
            ] = None
            ...
            if validate:
                if not (pattern or lang or config):
                    ...
                    (
                        resolved_configs,
                        config_errors,
                    ) = fastlint.config_resolver.get_config(
                        pattern,
                        lang,
                        config or [],
                    ...
                    if config_errors:
                        ...
                        raise FastlintError("Please fix the above errors and try again.")
            else:
                try:
                    (
                        ...
                        respect_fastlintignore=(not x_ignore_fastlintignore_files),
                        timeout=timeout,
                        max_memory=max_memory,
                        timeout_threshold=timeout_threshold,
                        interfile_timeout=interfile_timeout,
