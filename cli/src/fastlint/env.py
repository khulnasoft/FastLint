...
from fastlint.constants import SETTINGS_FILENAME
...
class Env:
    ...
    fail_open_url: str = field(
        default=EnvFactory(
            ["FASTLINT_FAIL_OPEN_URL"],
            "https://fail-open.prod.fastlint.dev",
        ),
        converter=migrate_fail_open_url,
    )
    fastlint_url: str = field(
        default=EnvFactory(["FASTLINT_URL", "FASTLINT_APP_URL"], "https://fastlint.dev"),
        converter=url,
    )
    app_token: Optional[str] = field(default=EnvFactory("FASTLINT_APP_TOKEN"))

    # Unique identifier for the managed_scan in fastlint-app
    sms_scan_id: Optional[str] = field(default=EnvFactory("FASTLINT_MANAGED_SCAN_ID"))

    version_check_url: str = field(
        default=EnvFactory(
            "FASTLINT_VERSION_CHECK_URL", "https://fastlint.dev/api/check-version"
        )
    )
    ...
    def version_check_timeout_default(self) -> int:
        value = os.getenv("FASTLINT_VERSION_CHECK_TIMEOUT", "2")
        return int(value)

    @version_check_cache_path.default
    def version_check_cache_path_default(self) -> Path:
        value = os.getenv("FASTLINT_VERSION_CACHE_PATH")
        if value:
            return Path(value)
        return Path.home() / ".cache" / "fastlint_version"

    @git_command_timeout.default
    def git_command_timeout_default(self) -> int:
        value = os.getenv("FASTLINT_GIT_COMMAND_TIMEOUT", "300")
        return int(value)

    @src_directory.default
    def src_directory_default(self) -> Path:
        value = os.getenv("FASTLINT_SRC_DIRECTORY")
        if value:
            return Path(value)
        return Path("/src")
    ...
    def user_data_folder_default(self) -> Path:
        ...
        return parent_dir / ".fastlint"

    @user_log_file.default
    def user_log_file_default(self) -> Path:
        path = os.getenv("FASTLINT_LOG_FILE", str(self.user_data_folder / "fastlint.log"))
        return Path(path)

    @user_settings_file.default
    def user_settings_file_default(self) -> Path:
        path = os.getenv(
            "FASTLINT_SETTINGS_FILE", str(self.user_data_folder / SETTINGS_FILENAME)
        )
        return Path(path)

    @in_docker.default
    def in_docker_default(self) -> bool:
        return "FASTLINT_IN_DOCKER" in os.environ
    ...
    def in_agent_default(self) -> bool:
        return "FASTLINT_AGENT" in os.environ

    @with_new_cli_ux.default
    def with_new_cli_default(self) -> bool:
        return os.environ.get("FASTLINT_NEW_CLI_UX", "0") == "1"
    ...
    def min_fetch_depth_default(self) -> int:
        value = os.getenv("FASTLINT_GHA_MIN_FETCH_DEPTH", "0")
        return int(value)

    @upload_findings_timeout.default
    def upload_findings_timeout_default(self) -> int:
        value = os.getenv("FASTLINT_UPLOAD_FINDINGS_TIMEOUT", "300")
        return int(value)
