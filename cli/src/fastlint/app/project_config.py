"""
Loading and saving of the .fastlintconfig.yml file.
"""
import re
...
from fastlint.git import get_git_root_path
from fastlint.verbose_logging import getLogger

logger = getLogger(__name__)

CONFIG_FILE_PATTERN = re.compile(r"^\.fastlintconfig(\.yml|\.yaml)?$")


@define
class ProjectConfig:
    """
    Class that handles loading and validating fastlintconfig files.
    ...
    def load_from_file(cls, file_path: Path) -> "ProjectConfig":
        ...
        logger.debug(f"Loading fastlintconfig file: {file_path}")
