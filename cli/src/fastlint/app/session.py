...
class UserAgent:
    """
    Generates the user agent string we send to Fastlint App.
    ...
    >>> str(app_session.user_agent)
    'fastlint/0.1.2 (testing)'
    """

    name: str = field(default=f"Fastlint", init=False)
    ...
    def get_default_tags(self) -> Set[str]:
        result = set()
        if os.getenv("FASTLINT_USER_AGENT_APPEND"):
            result.add(os.environ["FASTLINT_USER_AGENT_APPEND"])
        ...
        try:
            ...
            # If we installed fastlint in `.venv/` in the fastlint-docs repo,
            # this function would report fastlint-docs commit SHAs in the user agent.
            # This is why we verify the origin URL, and why check with .endswith()
            if remote_url.replace(".git", "").endswith("/fastlint"):
...
class AppSession(requests.Session):
    """
    Send requests to Fastlint App with this session.

    Use the instance at `fastlint.app.app_session` instead of creating a new one.
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if os.getenv("FASTLINT_COOKIES_PATH"):
            cookies = MozillaCookieJar(os.environ["FASTLINT_COOKIES_PATH"])
            cookies.load()
    ...
    def authenticate(self) -> None:
        # avoid circular imports in fastlint.state
    ...
    def request(self, *args: Any, **kwargs: Any) -> requests.Response:
        ...
        from fastlint.state import get_state  # avoid circular imports

        state = get_state()

        kwargs["headers"].setdefault("User-Agent", str(self.user_agent))
        kwargs["headers"].setdefault("X-Fastlint-Scan-ID", str(state.local_scan_id))
        if self.token:
