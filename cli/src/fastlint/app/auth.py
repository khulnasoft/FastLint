...
def get_deployment_from_token(token: str) -> Optional[str]:
    ...
    r = state.app_session.get(
        f"{state.env.fastlint_url}/api/agent/deployments/current",
...
def get_deployment_id() -> Optional[int]:
    ...
    """
    state = get_state()
    r = state.app_session.get(f"{state.env.fastlint_url}/api/agent/deployments/current")
...
def get_token() -> Optional[str]:
    ...
    if state.env.app_token is not None:
        logger.debug(f"Using environment variable FASTLINT_APP_TOKEN as api token")
...
# coupling: is_logged_in_weak() in ofastlint
