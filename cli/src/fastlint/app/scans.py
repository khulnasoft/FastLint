# Handle communication of findings / errors to fastlint.app
import json
import os
import sys
...
from fastlint.subproject import resolved_subproject_to_stats
from fastlint.subproject import subproject_to_stats
...
class ScanHandler:
    def __init__(
        ...
        """
        When dry_run is True, fastlint ci would get the config from the app,
        run the scans, but would not upload the results.

        When partial_output is not None, the scan results for fastlint ci
    ...
    @property
    def deepfastlint(self) -> bool:
        """
        Separate property for easy of mocking in test
        """
        if self.scan_response:
            return self.scan_response.engine_params.deepfastlint
        return False
    ...
    def start_scan(
        ...
        if response.status_code == 404:
            raise Exception(
                ...
                f"API server at {state.env.fastlint_url} returned this response: {response.text}"
            )
    ...
    def report_failure(self, exit_code: int) -> None:
        """
        Send fastlint cli non-zero exit code information to server
        and return what exit code fastlint should exit with.
        """
        state = get_state()
        ...
        if self.dry_run:
            logger.info(f"Would have reported failure to fastlint.dev: {exit_code}")
            return

        response = state.app_session.post(
            f"{state.env.fastlint_url}/api/agent/scans/{self.scan_id}/error",
            json={
                "exit_code": exit_code,
                "stderr": "",
            },
    ...
    def report_findings(
        ...
        # alt: save it in ~/.fastlint/logs/findings_and_ignores.json?
        logger.debug(f"Sending findings and ignores blob")

        results_task = progress_bar.add_task("Uploading scan results")
        response = state.app_session.post(
            f"{state.env.fastlint_url}/api/agent/scans/{self.scan_id}/results",
        ...
        while True:
            ...
            # alt: save it in ~/.fastlint/logs/complete.json?
            ...
            # mark as complete
            response = state.app_session.post(
                f"{state.env.fastlint_url}/api/agent/scans/{self.scan_id}/complete",
                timeout=state.env.upload_findings_timeout,
