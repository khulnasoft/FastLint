"""Ask the Fastlint App server about the latest Fastlint version

This module is for pinging the app to ask for the latest Fastlint release
...
# in the Fastlint App repository, and this is used just as a reference.
...
def _show_banners() -> None:
    ...
    if logged_something and env.in_agent:
        logger.warning(
            "If you're using the khulnasoft/fastlint-agent:v1 image, you will be automatically upgraded within 24 hours."
        )
...
def get_no_findings_msg() -> Optional[str]:
    ...
    """
    # only the real version_check request should be allowed to send a request to fastlint.dev
