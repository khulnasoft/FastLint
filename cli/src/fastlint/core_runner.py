...
from fastlint import tracing
from fastlint.app import auth
from fastlint.config_resolver import Config
from fastlint.console import console
from fastlint.constants import Colors
from fastlint.constants import PLEASE_FILE_ISSUE_TEXT
from fastlint.core_output import core_error_to_fastlint_error
from fastlint.core_output import core_matches_to_rule_matches
from fastlint.core_targets_plan import Plan
from fastlint.core_targets_plan import Task
from fastlint.engine import EngineType
from fastlint.error import FastlintCoreError
from fastlint.error import FastlintError
from fastlint.error import with_color
from fastlint.output_extra import OutputExtra
from fastlint.parsing_data import ParsingData
from fastlint.rule import Rule
from fastlint.rule_match import OrderedRuleMatchList
from fastlint.rule_match import RuleMatchMap
from fastlint.fastlint_types import Language
from fastlint.state import DesignTreatment
from fastlint.state import get_state
from fastlint.target_manager import TargetManager
from fastlint.target_mode import TargetModeConfig
...
def setrlimits_preexec_fn() -> None:
    ...
    Note this is intended to run as a preexec_fn before fastlint-core in a subprocess
    so all code here runs in a child fork before os switches to fastlint-core binary
    """
    ...
    # we use a different logger than the usual "fastlint" one
    core_logger = getLogger("fastlint_core")
    if IS_WINDOWS:
...
# from _invoke_fastlint(), see output.py and the management of self.error_set
def dedup_errors(errors: List[FastlintCoreError]) -> List[FastlintCoreError]:
    return list({uniq_error_id(e): e for e in errors}.values())


def uniq_error_id(error: FastlintCoreError) -> Any:
...
class StreamingFastlintCore:
    """
    Handles running fastlint-core in a streaming fashion

    This behavior is assumed to be that fastlint-core:
    ...
    capture_stderr is to capture the stderr of fastlint-core in a pipe; if set
    to false then the stderr of fastlint-core is reusing the one of pyfastlint
    allowing to show the logs of fastlint-core as soon as they are produced.
    """
    ...
    def __init__(
        ...
        """
        cmd: fastlint-core command to run
        ...
        # file system when servicing requests from fastlint-core.
        self.vfs_map: Dict[str, bytes] = {}

    @property
    def stdout(self) -> str:
        # stdout of fastlint-core sans "." and extra target counts
        return self._stdout

    @property
    def stderr(self) -> str:
        # stderr of fastlint-core command
        return self._stderr

    async def _core_stdout_processor(self, stream: asyncio.StreamReader) -> None:
        """
        Asynchronously process stdout of fastlint-core

        Updates progress bar one increment for every "." it sees from fastlint-core
        stdout

        Increases the progress bar total for any number reported from fastlint-core
        stdout
        ...
        while True:
            ...
            try:
                ...
                # that are looking for it. Make sure `fastlint-core exited with unexpected output`
                ...
                raise FastlintError(
                    ...
                    1. Increase the amount of memory available to fastlint
                    2. Reduce the number of jobs fastlint runs with via `-j <jobs>`. We
                        recommend using 1 job if you are running out of memory.
                    3. Scan the repo in parts (contact us for help)

                    Otherwise, it is likely that fastlint is hitting the limit on only some
                    files. In this case, you can try to set the limit on the amount of memory
                    fastlint can use on each file with `--max-memory <memory>`. We recommend
                    ...
                    contact us.
                       ...
                       Error: fastlint-core exited with unexpected output
            ...
            if line_bytes == b".\n" and not reading_json:
                ...
                #
                # However, for regular OSS Fastlint, we only print one dot per
    ...
    async def _core_stderr_processor(
        ...
        """
        Asynchronously process stderr of fastlint-core
    ...
    def _handle_read_file(self, fname: str) -> Tuple[bytes, int]:
        """
        Handler for fastlint_analyze 'read_file' callback.
        """
        try:
    ...
    async def _handle_process_outputs(
        ...
        for r in results:
            if isinstance(r, Exception):
                raise FastlintError(f"Error while running rules: {r}")

    async def _stream_exec_subprocess(self) -> int:
        """
        Run fastlint-core via fork/exec, consuming its output
        asynchronously.
        ...
        # spans nest under the correct pyfastlint parent span.
        ...
        # Usually happens when the process is killed by the OS
            ...
            # fastlint-core process was killed
            try:
                exit_code = await asyncio.wait_for(process.wait(), timeout=1.0)
            except TimeoutError:
                logger.error(
                    "fastlint timed out waiting for the fastlint-core process to exit after an exception was raised"
                )
                raise e

            # let's log this and reraise as if we got a non zero exit code with
            # a fastlint error then we segfaulted or OOMd and so should
            # immediately exit instead of assuming we got something usuable
            logger.error(f"fastlint-core exited with {exit_code}!")
            raise e
    ...
    def execute(self) -> int:
        """
        Run fastlint-core and listen to stdout to update
        progress_bar as necessary

        Blocks til completion and returns exit code
        """
        open_and_ignore(f"{tempfile.gettempdir()}/core-runner-fastlint-BEGIN")
        ...
        open_and_ignore(f"{tempfile.gettempdir()}/core-runner-fastlint-END")
        return rc


class CoreRunner:
    """
    Handles interactions between fastlint and fastlint-core

    This includes properly invoking fastlint-core and parsing the output
    """
    ...
    def _extract_core_output(
        ...
        if not core_stderr:
            core_stderr = (
                "<fastlint-core stderr not captured, should be printed above>\n"
            )

        # All paths in this block should call self._fail() to raise a
        # FastlintError, as something is wrong if fastlint-core's exit code is non zero!!
        if returncode != 0:
            ...
            if "errors" in output_json:
                ...
                fastlint_errors = None
                if len(errors) < 1:
                    fail_msg = (
                        "non-zero exit status errors array is empty in json response"
                    )
                else:
                    fastlint_errors = [core_error_to_fastlint_error(e) for e in errors]
                self._fail(
                    fail_msg,
                    shell_command,
                    returncode,
        ...
        # By default, we print fastlint-core's error output, which includes
        # fastlint-core's logging if it was requested via --debug.
        #
        # If fastlint-core prints anything on stderr when running with default
        # flags, it's a bug that should be fixed in fastlint-core.
        #
        ...
        logger.debug(
            f"--- fastlint-core stderr ---\n"
            f"{core_stderr}"
            f"--- end fastlint-core stderr ---"
        )
        ...
        # logger.debug(
        #     f"--- fastlint-core JSON answer ---\n"
        #     f"{output_json}"
        #     f"--- end fastlint-core JSON answer ---"
        # )
        # alt: save it in ~/.fastlint/logs/fastlint_core.json?
    ...
    def _parse_core_output(
        ...
        shell_command: str,
        fastlint_output: str,
        fastlint_error_output: str,
        returncode: int,
    ) -> Dict[str, Any]:
        # See if fastlint output contains a JSON error that we can decode.
        try:
            return cast(Dict[str, Any], json.loads(fastlint_output))
    ...
    def _fail(
        ...
        fastlint_output: str,
        fastlint_error_output: str,
        fastlint_errors: Optional[List[FastlintCoreError]] = None,
    ) -> None:
        """Raise a Fastlint Error that indicates fastlint-core ran into an error and exited
        abnormally. It will print the reason, shell_command, return code, and stdout/stderr of fastlint-core
        ...
        """
        expected_json_msg = "unexpected non-json output while invoking fastlint-core:"
        if fastlint_errors:
            errors_str = "\n".join([f"   {error}" for error in fastlint_errors])
            expected_json_msg = f"unexpected errors in json output after invoking fastlint-core:\n{errors_str}"

        # Once we require python >= 3.8, switch to using shlex.join instead
        # for proper quoting of the command line.
        details = with_color(
            Colors.white,
            f"fastlint-core exit code: {returncode}\n"
            f"fastlint-core command: {shell_command}\n"
            f"{expected_json_msg}\n"
            "--- fastlint-core stdout ---\n"
            f"{fastlint_output}"
            "--- end fastlint-core stdout ---\n"
            "--- fastlint-core stderr ---\n"
            f"{fastlint_error_output}"
            "--- end fastlint-core stderr ---\n",
        )
        raise FastlintError(
    ...
    def plan_core_run(
        ...
        *,
        ...
        Fastlint-core will use this to determine what to run
        (see fastlint_output_v1.atd and the target types).
    ...
    def _run_rules_direct_to_fastlint_core_helper(
        self,
        rules: List[Rule],
        target_manager: TargetManager,
        ...
        sca_subprojects: Dict[out.Ecosystem, List[out.ResolvedSubproject]],
    ) -> Tuple[RuleMatchMap, List[FastlintError], OutputExtra,]:
        state = get_state()
        logger.debug(f"Passing whole rules directly to fastlint_core")

        outputs: RuleMatchMap = collections.defaultdict(OrderedRuleMatchList)
        errors: List[FastlintError] = []
        ...
        # a future direct run of fastlint-core. This method of file management
        ...
        rule_file = exit_stack.enter_context(
            (state.env.user_data_folder / "fastlint_rules.json").open("w+")
        ...
        if not target_mode_config.is_historical_scan:
            target_file = exit_stack.enter_context(
                (state.env.user_data_folder / "fastlint_targets.txt").open("w+")
                if dump_command_for_core
        ...
        if target_mode_config.is_pro_diff_scan:
            diff_target_file = exit_stack.enter_context(
                (state.env.user_data_folder / "fastlint_diff_targets.txt").open("w+")
                if dump_command_for_core
                else tempfile.NamedTemporaryFile("w+", delete=(not IS_WINDOWS))
            )
        ...
        with exit_stack:
            if self._binary_path is None:
                if engine.is_pro:
                    logger.error(
                        f"""
Fastlint Pro is either uninstalled or it is out of date.

Try installing Fastlint Pro (`fastlint install-fastlint-pro`).
                        """
                    )
                else:
                    ...
                    logger.error(
                        f"""
Could not find the fastlint-core executable. Your Fastlint install is likely corrupted. Please uninstall Fastlint and try again.
                        """
                    )
                sys.exit(2)
            cmd = [
            ...
            # Create a map to feed to fastlint-core as an alternative to
            ...
            # need to replace the FastlintCore.path() part.
            if engine.is_pro:
                if auth.get_token() is None:
                    raise FastlintError(
                        "This is a proprietary extension of fastlint.\n"
                        "You must log in with `fastlint login` to access this extension."
                    )

                if engine is EngineType.PRO_INTERFILE:
                    logger.error(
                        "Fastlint Pro Engine may be slower and show different results than Fastlint OSS."
                    )
            ...
            logger.debug("Running Fastlint engine with command:")
            ...
            parsed_errors = [core_error_to_fastlint_error(e) for e in core_output.errors]
    ...
    def _run_rules_direct_to_fastlint_core(
        self,
        rules: List[Rule],
        target_manager: TargetManager,
        dump_command_for_core: bool,
        ...
        """
        Sometimes we may run into synchronicity issues with the latest DeepFastlint binary.
        These issues may possibly cause a failure if a user, for instance, updates their
        version of Fastlint, but does not update to the latest version of DeepFastlint.

        A short bandaid solution for now is to suggest that a user updates to the latest
        version, if the DeepFastlint binary crashes for any reason.
        """
        try:
            return self._run_rules_direct_to_fastlint_core_helper(
                rules,
                target_manager,
            ...
            # Handle Fastlint errors normally
            raise e
        except Exception as e:
...
Fastlint Pro crashed during execution (unknown reason).
This can sometimes happen because either Fastlint Pro or Fastlint is out of date.

Try updating your version of Fastlint Pro (`fastlint install-fastlint-pro`) or your version of Fastlint (`pip install fastlint/brew install fastlint`).
If both are up-to-date and the crash persists, please contact support to report an issue!
When reporting the issue, please re-run the fastlint command with the
...
Exception raised: `{e}`
    ...
    # end _run_rules_direct_to_fastlint_core

    def invoke_fastlint_core(
        self,
        target_manager: TargetManager,
        rules: List[Rule],
        dump_command_for_core: bool,
        ...
        (
            ...
            errors,
            output_extra,
        ) = self._run_rules_direct_to_fastlint_core(
            rules,
            target_manager,
            dump_command_for_core,
            time_flag,
        ...
        logger.debug(
            f"fastlint ran in {datetime.now() - start} on {len(output_extra.all_targets)} files"
        )
    ...
    def validate_configs(self, configs: Tuple[str, ...]) -> Sequence[FastlintError]:
        if self._binary_path is None:  # should never happen, doing this for mypy
            raise FastlintError("fastlint engine not found.")

        metachecks = Config.from_config_list(
            ["p/fastlint-rule-lints"], None, force_jsonschema=True
        )[0].get_rules(True)
        ...
        with tempfile.NamedTemporaryFile(
            ...
            parsed_errors += [
                core_error_to_fastlint_error(e) for e in core_output.errors
            ]
