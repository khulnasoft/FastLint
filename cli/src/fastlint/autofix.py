...
#
# This uses an RPC mechanism to call out to `fastlint-core` separately from the
# main call to `fastlint-core`. In https://github.com/khulnasoft/fastlint/pull/9229,
# Brandon initially attempted to move autofix application into the main call to
# `fastlint-core`. However, logic implemented in Python after that call relies on
# the repository being in its original state. For example, we run a baseline
# scan after the initial call to `fastlint-core`, which requires the repository
# to be in a clean state. Therefore, we cannot currently absorb this
# functionality into the main `fastlint-core` call.
def apply_fixes(rule_matches_by_rule: RuleMatchMap, dryrun: bool = False) -> None:
    ...
    application_result = fastlint.rpc_call.apply_fixes(rpc_args)
