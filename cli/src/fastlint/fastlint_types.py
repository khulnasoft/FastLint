...
from fastlint.error import UnknownLanguageError
from fastlint.error_location import Span
from fastlint.types import JsonObject
...
def get_frozen_id() -> UUID:
    ...
    within the Fastlint App. We avoid initializing this UUID within our state object to
...
class LanguageDefinition:
    """
    Mirrors schema of lang.json (see fastlint_interfaces/README.md) for each language
    """
...
class _LanguageData:
    def __init__(self) -> None:
        with (Path(__file__).parent / "fastlint_interfaces" / "lang.json").open() as fd:
            data = json.load(fd)
    ...
    def resolve(self, lang_str: str, span: Optional[Span] = None) -> Language:
        ...
        :param span: span of language string in the original file (for error reporting),
                    None if resolve was called within fastlint
        """
        ...
        if normalized in self.lang_by_key:
            ...
            raise UnknownLanguageError(
                ...
                long_msg=f"unsupported language: {normalized}. {self.show_suppported_languages_message()}\n\nYou may need to update your version of Fastlint, if you are on an old version that does not yet support this language.",
                spans=spans,
            )
