 ...
 *  - feather (shell DSL)
 *  - some Fastlint libraries! (I need my Common)
 *
 * usage:
...
(* This assumes you did a 'make install-fastlint-libs' so that
 * the fastlint libraries are available from topfind.
 *)
#require "commons"
...
(* We don't use Cmd here so we can also use fastlint/libs/commons/Cmd.ml *)
